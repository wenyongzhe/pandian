...
public class SQLStockDataSqlite extends SQLDataSqlite {
	
	private String BarSplitLenSetTableName = "BarSplitLenSet";
	private String BoxDetailTableName = "BoxDetail";
	private String CheckDetailTableName = "CheckDetail";
	private String CheckDetailSingleTableName = "CheckDetailSingle";
	private String CheckMainTableName = "CheckMain";
	private String CreateAndDelLogTableName = "CheckAndDelLog";
	private String ExportSetInfoTableName = "ExportSetInfo";
	private String GdInfoTableName = "GdInfo";
	private String ImportSetInfoTableName = "ImportSetInfo";
	private String LengthSetTableName = "LengthSet";
	private String UsersTableName = "Users";
	protected static StockDataOpenHelper helper = null;
	protected static LocalStockDataOpenHelper helperLocal = null;


	public SQLStockDataSqlite(Context context) {
		this.context = context;
		helper = StockDataOpenHelper.getInstance(context);
	}

	public SQLStockDataSqlite(Context context, boolean isWrite) {
		this.context = context;
		helper = StockDataOpenHelper.getInstance(context);
		if (isWrite) {
	...
	public void saveExportSet(ExportSet exportSet) {
		String strSql = "delete from ExportSetInfo";
		db.beginTransaction();
		try {
			db.execSQL(strSql);
			strSql = "insert into ExportSetInfo values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
	...
	public List<ExportSet> getExportSetList() {
		...
		Cursor cursor = null;
		String strSql = "select * from ExportSetInfo";
		db.beginTransaction();
		try {
			cursor = db.rawQuery(strSql, null);
			while (cursor.moveToNext()) {
	...
	public int DeleteDetailData(String strType, CheckDetail checkDetail) {
		...
		try {
			String strSql = "";
			if (strType.equals("0")) {
				strSql = "delete from CheckDetail where ShopID=? and CheckID=?";
				db.execSQL(strSql, new Object[] { checkDetail.strShopID,
						checkDetail.strCheckID });
			} else {
				strSql = "delete from CheckDetail where ShopID=? and CheckID=? and PositionID=?";
				db.execSQL(strSql, new Object[] { checkDetail.strShopID,
						checkDetail.strCheckID, checkDetail.strPositionID });
			}
		} catch (Exception ex) {
			iRet = 0;
			Utility.deBug("CheckDetail DeleteDetailData Error:",
					ex.getMessage());
		}
		return iRet;
	}
	...
	/**获取导出数据，拆分条码形式为条码+1**/
	public List<String[]> GetExportDatasSplitBarcode(String strExportType,List<STROrder> strOrders, String strCheckID,String strShopID, String strPositionID) {
		...
		try {
			StringBuffer buffer = new StringBuffer();
				...
				String sName = buffer.toString().substring(0, buffer.toString().length() - 1);
				LogUtil.i(sName);
				if (strExportType.equals("0")) {
					strSql = "select " + sName + " From CheckDetail"
					...
					while (cursor.moveToNext()) {
						...
						for(int i = 0; i < strOrders.size(); i++){
							str[i]=cursor.getString(cursor.getColumnIndex(strOrders.get(i).strOriName));
			...
			Utility.deBug("CheckDetail GetExportCustom Method Error",
					ex.getMessage());
		} finally {
			db.endTransaction();
			cursor.close();
	...
	public List<String[]> GetExportDatas(String strExportType,List<STROrder> strOrders, String strCheckID,String strShopID, String strPositionID) {
//ID ShopID CheckID PositionID GdBar CheckNum GdArtNO GdStyleGdColorID 
//		GdSizeID CheckTime GdName GdColorName GdSizeName Property1 Property2 GdStock GdPrice 
		//List<String[]> stringList= new ArrayList<String[]>();
		List<String[]> stringList= new ArrayList<String[]>();
		String[] str=new String[strOrders.size()];
		String strSql = "";
	...
	/**导出数据根据数量拆分为一条一条**/
	public List<String[]> GetExportDatasByScanOrderSplitBarcode(String strExportType,
		...
		try {
			...
			if (strExportType.equals("0")) {
				strSql = "select ShopID," + buffer
						+ "CheckNum,CheckTime From CheckDetail"
		...
		public float GetSpeTotalQty(String strExportType, String strShopID,
			...
			try {
				...
				{
					strSql = "select CheckNum From CheckDetail where ShopID=? And CheckID=?";
					cursor = db.rawQuery(strSql, new String[] { strShopID,
							strCheckID, strPositionID });
					while (cursor.moveToNext()) {
				...
				{
					strSql = "select CheckNum From CheckDetail where ShopID=? And CheckID=? And PositionID=?";
					cursor = db.rawQuery(strSql, new String[] { strShopID,
							strCheckID ,strPositionID });
					while (cursor.moveToNext()) {
						float checkNum = cursor.getFloat(cursor
				...
				{
					strSql = "select CheckNum from CheckDetail where ShopID=?";
					cursor = db.rawQuery(strSql, new String[] { strShopID });
					while (cursor.moveToNext()) {
						float checkNum = cursor.getFloat(cursor
								.getColumnIndex("CheckNum"));
				...
				Utility.deBug("CheckDetail ExistSpecifyBar Method Error",
						ex.getMessage());
				// dTotal = -1M;
			} finally {
				db.endTransaction();
	...
	// / 获取导出时的定制数据
	// / </summary>
	// / <param name="iWay">获取数据的方式，0为按编号获取，1为按编号，货位获取，2为按门店获取</param>
	// /
//	public List<ExportCustomiseInfo> GetExportCustomiseData(int iWay,
//			String strCheckID, String strShopID, String strPositionID) {
//		List<ExportCustomiseInfo> exportDataList = new ArrayList<ExportCustomiseInfo>();
//		String strSql = "";
//		Cursor cursor = null;
//		db.beginTransaction();
//		try {
...
//			if (iWay == -1) {
//
//				strSql = "select GdBar,CheckTime from CheckDetail where ShopID=?";
//				cursor = db.rawQuery(strSql, new String[] { strShopID });
//				while (cursor.moveToNext()) {
//					ExportCustomiseInfo exportDataInfo = new ExportCustomiseInfo();
//					exportDataInfo.GdBar = cursor.getString(cursor
//							.getColumnIndex("GdBar"));
//					exportDataInfo.checkTime = cursor.getString(cursor
//							.getColumnIndex("CheckTime"));
//					String strSql1 = "select CheckNum from CheckDetail where ShopID=? and GdBar=? group by GdBar";
//					int sum = 0;
//					String CheckTime;
//					Cursor cursor1 = db.rawQuery(strSql1, new String[] {
//							strShopID, exportDataInfo.GdBar });
//					while (cursor1.moveToNext()) {
//
//						int checkNum = cursor1.getInt(cursor1
//								.getColumnIndex("CheckNum"));
//						sum += checkNum;
//					}
//
//					cursor1.close();
//					exportDataInfo.CheckNum = sum;
//					exportDataList.add(exportDataInfo);
//
//				}
//
//			} else if (iWay == 0) {
//				strSql = "select GdBar,CheckTime from CheckDetail where ShopID=? and CheckID=?";
//				cursor = db.rawQuery(strSql, new String[] { strShopID,
//						strCheckID });
//				while (cursor.moveToNext()) {
//					ExportCustomiseInfo exportDataInfo = new ExportCustomiseInfo();
...
//					exportDataInfo.checkTime = cursor.getString(cursor
//							.getColumnIndex("CheckTime"));
//					String strSql1 = "select CheckNum from CheckDetail where CheckID=? and ShopID=? and GdBar=? group by GdBar";
//					int sum = 0;
//					Cursor cursor1 = db.rawQuery(strSql1, new String[] {
//							strCheckID, strShopID, exportDataInfo.GdBar });
//					while (cursor1.moveToNext()) {
//
//						int checkNum = cursor1.getInt(cursor1
//								.getColumnIndex("CheckNum"));
...
//				}
//			} else if (iWay == 1) {
//				strSql = "select GdBar,CheckTime From CheckDetail where ShopID=? and CheckID=? and PositionID=? group by GdBar";
...
//					exportDataInfo.checkTime = cursor.getString(cursor
//							.getColumnIndex("CheckTime"));
//					strSql = "select CheckNum from CheckDetail where CheckID=? and PositionID=? and ShopID=? and GdBar=? group by GdBar";
//					int sum = 0;
//					Cursor cursor1 = dbLocal.rawQuery(strSql, new String[] {
//							strCheckID, strPositionID, strShopID,
//							exportDataInfo.GdBar });
//					while (cursor1.moveToNext()) {
//						int checkNum = cursor1.getInt(cursor
//								.getColumnIndex("CheckNum"));
//						sum += checkNum;
//					}
//					cursor1.close();
...
//				}
//			} else if (iWay == 2) {
//				strSql = "select GdBar,CheckTime From CheckDetail where ShopID=? group by GdBar";
//				cursor = dbLocal.rawQuery(strSql, new String[] { strShopID });
//				while (cursor.moveToNext()) {
//					ExportCustomiseInfo exportDataInfo = new ExportCustomiseInfo();
//					exportDataInfo.GdBar = cursor.getString(cursor
...
//					exportDataInfo.checkTime = cursor.getString(cursor
//							.getColumnIndex("CheckTime"));
//					strSql = "select CheckNum from CheckDetail where  ShopID=? and GdBar=? group by GdBar";
//					int sum = 0;
//					Cursor cursor1 = dbLocal.rawQuery(strSql, new String[] {
//							strShopID, exportDataInfo.GdBar });
//					while (cursor1.moveToNext()) {
...
//	}
	...
	public int DeleteSingleData(String strType, String strCheckID,
		...
		try {
			...
			String strSqlSingle = "";
			if (strType.equals("0")) {
				strSqlMain = "delete from CheckMain where ShopID=? and CheckID=?";
				strSqlDetail = "delete from CheckDetail where ShopID=? and CheckID=?";
				strSqlSingle = "delete from CheckDetailSingle where ShopID=? and CheckID=?";
				...
				strSqlMain = "delete from CheckMain where ShopID=? and CheckID=? and PositionID=?";
				strSqlDetail = "delete from CheckDetail where ShopID=? and CheckID=? and PositionID=?";
				strSqlSingle = "delete from CheckDetailSingle where ShopID=? and CheckID=? and PositionID=?";
				db.execSQL(strSqlMain, new Object[] { strShopID, strCheckID,
						strPositionID });
				db.execSQL(strSqlDetail, new Object[] { strShopID, strCheckID,
						strPositionID });
	...
	public int DeleteAllDetailData(String shopID) {
		int iRet = 1;
		String strSql = "delete from CheckDetail where ShopID=?";
		try {
			db.execSQL(strSql, new Object[] { shopID });
			dbLocal.execSQL(strSql, new Object[] { shopID });
		} catch (Exception ex) {
			iRet = 0;
			Utility.deBug("CheckDetail DeleteAllDetailData Error:",
					ex.getMessage());
		}
		return iRet;
	}
	...
	public int DeleteAllCheckData(String strShopID) {
		int iRet = 1;
		String strSqlMain = "delete from CheckMain where ShopID=?";
		String strSqlDetail = "delete from CheckDetail where ShopID=?";
		String strSqlDetailSingle = "delete from CheckDetailSingle where ShopID=?";
		try {
	...
	public int DeleteAllShops() {
		int iRet = 1;
		String strSqlMain = "delete from CheckMain";
		String strSqlDetail = "delete from CheckDetail";
		String strSqlDetailSingle = "delete from CheckDetailSingle";
		try {
	...
	// 插入详情数据
		...
		public int InsertCheckDetailBySpecial(CheckDetail checkDetail, String usingBarCode , String valitedMethod,Boolean status,String isInputNum,int specialStatus) {
			int iRet = 0;
			String strSql = "insert into CheckDetail (ShopID,CheckID,PositionID,GdBar,CheckNum,GdArtNO,GdStyle,GdColorID,GdSizeID,"
					+ "GdName,GdColorName,GdSizeName,Property1,Property2,GdStock,GdPrice,ReservedMg,CheckTime)"
					+ " values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
			db.beginTransaction();
			int i = checkUniqueCode(checkDetail.strShopID, checkDetail.strCheckID, checkDetail.strGdBar);
			int j = checkUniqueCodeFromCheckDetail(checkDetail.strShopID, checkDetail.strCheckID,checkDetail.strPositionID, checkDetail.strGdBar);
			try {
				...
				//2是设置提示的情况
					...
					if (i != 1) {
						...
						//如果CheckDetail中有此条码时
							...
							//获取原有的数量,每次提示,点击确定 + 1
							int num = GetOriginalNum(checkDetail.strShopID,checkDetail.strCheckID,checkDetail.strPositionID,checkDetail.strGdBar);							
							checkDetail.dCheckNum = num  +  1;
							strSql = "Update CheckDetail Set CheckNum=? where ShopID=? And CheckID=? and PositionID=? And GdBar=?";
							db.execSQL(strSql,
					...
					if (isInputNum.equals("1")) {
						...
						//如果CheckDetail中有此条码时
							...
							strSql = "Update CheckDetail Set CheckNum=? where ShopID=? And CheckID=? and PositionID=? And GdBar=?";
							db.execSQL(strSql,
									new Object[] {checkDetail.dCheckNum, checkDetail.strShopID,
											checkDetail.strCheckID, checkDetail.strPositionID,											
											checkDetail.strGdBar });
							dbLocal.execSQL(strSql,
									new Object[] {checkDetail.dCheckNum, checkDetail.strShopID,
				...
				if (usingBarCode.equals("1")) {
					...
					String strSqlSingle = "Insert Into CheckDetailSingle(ShopID,CheckID,PositionID,"
				...
				iRet = 0;
				Utility.deBug("CheckDetail SqlInsert Method Error", ex.getMessage());
	 ...
	 * 插入详情数据
	 * @param checkDetail
	 * @param usingBarCode 唯一码
	 * @param valitedMethod 设置是否提示
	 * @param status
	 * @param isInputNum
	 * @param specialStatus
	 * @return
	 */
	public int InsertCheckDetail(CheckDetail checkDetail, String usingBarCode , String valitedMethod,Boolean status,String isInputNum,int specialStatus) {
		int iRet = 0;
		String strSql = "insert into CheckDetail (ShopID,CheckID,PositionID,GdBar,CheckNum,GdArtNO,GdStyle,GdColorID,GdSizeID,"
				+ "GdName,GdColorName,GdSizeName,Property1,Property2,GdStock,GdPrice,ReservedMg,CheckTime)"
				+ " values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
	...
	public int GetOriginalNum(String strShopID, String strCheckID,String strPositionID, String strGdBar) {
		int dAlreadyQty = 0;
		String strSql = "select CheckNum From CheckDetail where ShopID=? And CheckID=? and PositionID=? and GdBar=?";
		Cursor cursor = null;
		db.beginTransaction();
		try {
			...
			Utility.deBug("CheckDetail GetOriginalNum Method Error",ex.getMessage());
		} finally {
			db.endTransaction();
			cursor.close();
		}
	...
	public List<ScanOrderInfo> GetRecordsScanOrder(String strShopID,
		...
		String strSql = "select ID,GdBar,CheckNum from CheckDetail where ShopID =?"
...
//				}
	...
	// 编号查询
	public List<StaPositionSummary> GetStaPositionSummary(String strShopID,String strCheckID) {
		List<StaPositionSummary> staPositionSummaryList = new ArrayList<StaPositionSummary>();
		String strSql = "select PostionID from CheckDetail where shopID=? and CheckID=? order by PositionID";
		Cursor cursor = null;
		float sums = 0;
		db.beginTransaction();
		try {
			...
			while (cursor.moveToNext()) {
				...
				while (cursor1.moveToNext()) {
					float checkNum = cursor.getFloat(cursor.getColumnIndex("CheckNum"));
					sums += checkNum;
				}
				cursor1.close();
	...
	public List<StaCheckNODetail> GetStaCheckNODetail(String strShopID,
		...
		Cursor cursor = null;
		String strSql = "select ID,PositionID,GdBar,CheckNum from CheckDetail where ShopID=? and CheckID=? order by PositionID,GdBar";
		db.beginTransaction();
		try {
			cursor = db
			...
			while (cursor.moveToNext()) {
				...
				staCheckNODetail.Sums = cursor
						.getFloat(cursor.getColumnIndex("CheckNum"));
	...
	public List<CheckListEntity> GetStaCheckPositionDetail(String strShopID,
		...
		Cursor cursor = null;
		String strSql = "select GdBar from CheckDetail where ShopID=? and CheckID=? and PositionID=? order by CheckTime desc";
		db.beginTransaction();
		try {
			cursor = db.rawQuery(strSql, new String[] { strShopID, strCheckID,
					strPositionID });
			while (cursor.moveToNext()) {
				...
				Cursor cursor1 = db
						.rawQuery(
								"select CheckNum from CheckDetail where GdBar=? and ShopID=? and CheckID=? and PositionID=?",
	...
	public List<StaCheckDetail> GetStaCheckDetail(String strShopID,
		...
		Cursor cursor = null;
		String strSql = "Select ID,PositionID from CheckDetail where ShopID=? and CheckID=? and GdBar=? order by PositionID";
		db.beginTransaction();
		try {
			cursor = db.rawQuery(strSql, new String[] { strShopID, strCheckID,
					strGdBar });
			while (cursor.moveToNext()) {
				...
				float sums = 0;
				Cursor cursor1 = db.rawQuery(
						"select CheckNum from CheckDetail where PositionID=?",
	...
	public int ExistSpecifyBar01(String strShopID, String strCheckID,
		...
		int iRet = 0;
		Cursor cursor = null;
		String strSql = "select * from CheckDetailSingle where ShopID=? And CheckID=? and GdBarSingle=? and CheckNum > 0";
		db.beginTransaction();
		try {
			cursor = db.rawQuery(strSql, new String[] { strShopID, strCheckID,
					strSingleGdBar });
	...
	public int checkUniqueCode(String strShopID, String strCheckID,String strSingleGdBar) {
		int iRet = 0;
		Cursor cursor = null;
		String strSql = null;
		//内部码
	...
	public int checkUniqueCodeFromCheckDetail(String strShopID, String strCheckID,String strSingleGdBar) {
		int iRet = 0;
		Cursor cursor = null;
		String strSql = "select GdArtNO from CheckDetail where GdBar=? and ShopID = ?  and CheckID = ?";
		db.beginTransaction();
		try {
			cursor = db.rawQuery(strSql, new String[] {strSingleGdBar,strShopID,strCheckID});
			if (cursor.moveToFirst()) {
				iRet = 1;
			}
			db.setTransactionSuccessful();
			...
			iRet = 0;
			Utility.deBug("CheckDetail ExistSpecifyBar Method Error",ex.getMessage());
		} finally {
			db.endTransaction();
			cursor.close();
		}
	...
	public int checkUniqueCodeFromCheckDetail(String strShopID, String strCheckID,String strPoistion,String strSingleGdBar) {
		int iRet = 0;
		Cursor cursor = null;
		String strSql = "select GdArtNO from CheckDetail where GdBar=? and ShopID = ? and  PositionID=? and CheckID = ?";
		db.beginTransaction();
		try {
			cursor = db.rawQuery(strSql, new String[] {strSingleGdBar,strShopID,strPoistion,strCheckID});
			if (cursor.moveToFirst()) {
				iRet = 1;
			}
			db.setTransactionSuccessful();
	...
	// /// <param name="iCounter"></param>
	// /// <returns></returns>
	public int ExistSpecifyBar02(String strShopID, String strCheckID,String strPositionID, String strSingleGdBar) {
		int iRet = 0;
		Cursor cursor = null;
		String strSql = "Select * From CheckDetailSingle where ShopID=? And CheckID=? and PositionID=? and GdBarSingle=? And CheckNum > 0";
		db.beginTransaction();
		try {
			cursor = db.rawQuery(strSql, new String[] { strShopID, strCheckID,
...
/**
 * 
 * @param dOldNum 处理该次操作之前的盘点的数量
 * @param checkDetail
 * @param uniqueStatus
 * @param isAddQuantity 为当为逐行显示,并且要修改数量时
 * @param choosedQuantity 为当为逐行显示时,点击要被修改条码的数量,点击“修改数量”按钮的时候才用到
 * @param isBtn 区分是点击修改数量按钮，还是跳转修改数量
 * @return
 */
	// /// <summary>
	...
	public int SaveCheckDetail(int dOldNum, CheckDetail checkDetail,int uniqueStatus,boolean isAddQuantity,int choosedQuantity,boolean isBtn) {
		...
		try {
			if (checkDetail.dCheckNum == 0) {
				strSql = "delete from CheckDetail where ShopID=? And CheckID=? and PositionID=? And GdBar=?";
				...
				String sqlString = "select count(*) from CheckDetail where ShopID=? And CheckID=? and PositionID=? and GdBar=?";
				...
				while (condition.moveToNext()) {
					...
					if (condition.getInt(condition.getColumnIndex("count(*)")) == 0) {
						String test = "Insert Into CheckDetail(ShopID,CheckID,PositionID,GdBar,CheckNum,GdArtNO,GdStyle,GdColorID,GdSizeID,GdName,GdColorName,GdSizeName,Property1,Property2,GdStock,GdPrice,ReservedMg,CheckTime)" + "values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
						db.execSQL(test, new Object[] { checkDetail.strShopID,
								checkDetail.strCheckID,
								...
								checkDetail.dGdPrice,checkDetail.strReservedMg,
								Utility.getCurTime()});
					} else if (condition.getInt(condition.getColumnIndex("count(*)")) > 0) {
						//当为逐行显示的时候
						if (sp.getString(ConfigEntity.ScanningShowModeKey, ConfigEntity.ScanningShowMode).equals("1")&&isBtn) {
							
							if (isAddQuantity == true) {
								checkDetail.dCheckNum = dOldNum + checkDetail.dCheckNum - choosedQuantity;
							}
						...
						if(!isBtn){
							//不屏蔽：修改数量累加
							if (sp.getString(ConfigEntity.ScanningShowModeKey, ConfigEntity.ScanningShowMode).equals("0") && sp.getString(ConfigEntity.IsPutInNumKey, ConfigEntity.IsPutInNumKey).equals("0")) {
								checkDetail.dCheckNum = dOldNum + checkDetail.dCheckNum;
							}else if (sp.getString(ConfigEntity.ScanningShowModeKey, ConfigEntity.ScanningShowMode).equals("1") && sp.getString(ConfigEntity.IsPutInNumKey, ConfigEntity.IsPutInNumKey).equals("0")) {
								checkDetail.dCheckNum = dOldNum + checkDetail.dCheckNum;
							}
						}
						
						
						strSql = "Update CheckDetail Set CheckNum=? , CheckTime = ? where ShopID=? And CheckID=? and PositionID=? And GdBar=? ";
						db.execSQL(strSql,
			...
			if (checkDetail.dCheckNum == 0) {
				...
				strSql = "Insert Into CheckDetail(ShopID,CheckID,PositionID,GdBar,CheckNum,GdArtNO,GdStyle,GdColorID,GdSizeID,"
						+ "GdName,GdColorName,GdSizeName,Property1,Property2,GdStock,GdPrice, ReservedMg,CheckTime) "
						+ "values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
				dbLocal.execSQL(strSql, new Object[] { checkDetail.strShopID,
						checkDetail.strCheckID, checkDetail.strPositionID,
			...
			if (sp.getString(ConfigEntity.UsingBarCodeKey,
				...
				if (checkDetail.dCheckNum < dOldNum) {
					String strSql1 = "select ID from CheckDetailSingle where ShopID=? And CheckID=? and PositionID=? And GdBarSingle=?";
					cursor = db.rawQuery(strSql1, new String[] {
							checkDetail.strShopID, checkDetail.strCheckID,
							checkDetail.strPositionID,
							checkDetail.strSingleGdBar });
					while (cursor.moveToNext()) {
						strSqlSingle = "delete from CheckDetailSingle where ID =?";
					...
					strSqlSingle = "Insert Into CheckDetailSingle(ShopID,checkID,PositionID,GdBar,CheckNum,"
			...
			iRet = 0;
			Utility.deBug("CheckDetail SqlCheckAndInsert Method Error",
					ex.getMessage());
		} finally {
			db.endTransaction();
			dbLocal.endTransaction();
		...
		try {
			if (false)// GlobalRunConfig.GetInstance().iScanDisplayWay == 0)
			{
				strSql = "Select a.GdBar,a.CheckNum,a.GdArtNO,"
						...
						+ "a.Property1,a.Property2,a.GdStock,a.GdPrice,a.ID From CheckDetailSingle a where a.ShopID=?"
						...
						ConfigEntity.InOutCode).equals("1")) // 内部码
				{
					strSql = "Select  a.GdBar,a.CheckNum,a.GdArtNO,a.GdStyle,a.GdName,a.GdColorID,"
							...
							+ "From CheckDetail a where a.ShopID=?"
							+ "And a.CheckID=? and a.PositionID=? order by ID desc Limit "
							+ iRows;
			...
			Utility.deBug("CheckDetail GetSpecifiedQtyRecords Method Error",
					ex.getMessage());
		} finally {
			db.endTransaction();
			if (null != cursor) {
	...
	public float GetSpecifiedCheckSum(String strShopID, String strCheckID) {
		float dCheckSum = 0;
		Cursor cursor = null;
		String strSql = "Select CheckNum From CheckDetail where ShopID=? And CheckID=?";
		db.beginTransaction();
		try {
			cursor = db.rawQuery(strSql, new String[] { strShopID, strCheckID});
			while (cursor.moveToNext()) {
			...
			Utility.deBug("CheckDetail GetSpecifiedSum Method Error",
					ex.getMessage());
		} finally {
			db.endTransaction();
			cursor.close();
	...
	public int GetSpecifiedPositionSum(String strShopID, String strCheckID,
		...
		int dCheckSum = 0;
		Cursor cursor = null;
		String strSql = "Select CheckNum From CheckDetail where ShopID=? And CheckID=? and PositionID=?";
		db.beginTransaction();
		try {
			cursor = db.rawQuery(strSql, new String[] { strShopID, strCheckID,
					strPositionID });
	...
	//根据编号获取数量
	public int GetSpecifiedCheckIDSum(String strShopID, String strCheckID,String strPositionID) {
		int dCheckSum = 0;
		// dPositionSum = 0;
		Cursor cursor = null;
		String strSql = "Select sum(CheckNum) From CheckDetail where ShopID=? And CheckID=?";
		db.beginTransaction();
		try {
			cursor = db.rawQuery(strSql, new String[] { strShopID, strCheckID,});
			if (cursor.moveToFirst()) {
				 dCheckSum = cursor.getInt(cursor.getColumnIndex("sum(CheckNum)"));
			}
			db.setTransactionSuccessful();
		} catch (Exception ex) {
			Utility.deBug("CheckDetail GetSpecifiedSum Method Error",
	...
	//根据货位获取数量
		public int GetSpecifiedPositionIdSum(String strShopID, String strCheckID,String strPositionID) {
			int dCheckSum = 0;
			// dPositionSum = 0;
			Cursor cursor = null;
			String strSql = "Select sum(CheckNum) From CheckDetail where ShopID=? And PositionId=? And CheckId = ?";
			db.beginTransaction();
			try {
				cursor = db.rawQuery(strSql, new String[] { strShopID, strPositionID,strCheckID});
				if (cursor.moveToFirst()) {
					 dCheckSum = cursor.getInt(cursor.getColumnIndex("sum(CheckNum)"));
				}
				db.setTransactionSuccessful();
			} catch (Exception ex) {
				Utility.deBug("CheckDetail GetSpecifiedSum Method Error",ex.getMessage());
			} finally {
				db.endTransaction();
				cursor.close();
			}
	 ...
	 * @return
	 */
	public int GetGoodsQty(String strShopID, String strCheckID,String strPositionID, String strGdBar) {
		Cursor cursor1 = null;
		int dQty = 0;
		String strSql = "Select CheckNum from CheckDetail where ShopID=? and CheckID=? and PositionID=? and GdBar = ?";
		db.beginTransaction();
		try {
			cursor1 = db.rawQuery(strSql, new String[] { strShopID, strCheckID,strPositionID, strGdBar });
			while (cursor1.moveToNext()) {
				float checkNum = cursor1.getFloat(cursor1
						.getColumnIndex("CheckNum"));
				dQty += checkNum;
			...
			Utility.deBug("CheckDetail GetGoodsQty Method Error",ex.getMessage());
		} finally {
			db.endTransaction();
			if (cursor1 != null) {
				cursor1.close();
	...
	public CheckDetail GetSpeCheckRecord(String strShopID, String strCheckID,
		...
		try {
			...
			Utility.deBug("CheckDetail GetSpeCheckRecord Method Error:",
					ex.getMessage());
		} finally {
			db.endTransaction();
	...
	public int SaveCheckMain(String strShopID, String strCheckID,
			String strPositionID) {
		int iRet = 0;
		String strSql = "insert into CheckMain(ShopID,CheckID,PositionID,CheckTime) values(?,?,?,?)";
		db.beginTransaction();
		try {
			...
			iRet = 0;
			Utility.deBug("CheckMain SqlInsert Error", ex.getMessage());
		} finally {
			db.endTransaction();
		}
		return iRet;
	...
	public MainSummaryInfo GetSummaryQty(String strShopID) {
		...
		String strSql = "select a.CheckID,a.PositionID,"
				+ " from CheckMain a Left Join CheckDetail b on a.ShopID=b.ShopID and a.CheckID=b.CheckID and a.PositionID=b.PositionID "
		...
		try {
			...
			while (cursor.moveToNext()) {
				...
				summaryInfo.PositionID = cursor.getString(cursor
						.getColumnIndex("PositionID"));
				Cursor cursor1 = db.rawQuery(
						"select CheckNum from CheckDetail where CheckID=? and "
		...
		try {
			...
			while (cursor.moveToNext()) {
				...
				summaryInfo.PositionID = cursor.getString(cursor
						.getColumnIndex("PositionID"));
				Cursor cursor1 = db.rawQuery(
						"select CheckNum from CheckDetail where ShopID=? and CheckID=? and "
								+ "PositionID=?", new String[] {strShopID,
								summaryInfo.CheckID, summaryInfo.PositionID });
				float Qty = 0;
				while (cursor1.moveToNext()) {
					float checkNum = cursor1.getFloat(cursor1.getColumnIndex("CheckNum"));
					Qty += checkNum;
				}
				cursor1.close();
				summaryInfo.Qty = Qty;
	...
	public List<CheckMain> GetMainData(String strType, String strShopID) {
		...
		Cursor cursor = null;
		if (strType.equals("0")) {
			strSql = "select distinct CheckID from CheckMain where ShopID=?";
		} else {
			strSql = "select distinct CheckID,PositionID from CheckMain where ShopID=?";
		}
		db.beginTransaction();
		try {
			cursor = db.rawQuery(strSql, new String[] { strShopID });
	...
	public List<String> GetSpeCheckIDPositionID(String strShopID,
		...
		String strSql = "select distinct PositionID from CheckMain where ShopID=? and CheckID=?'";
		Cursor cursor = null;
		db.beginTransaction();
		try {
			cursor = db
	...
	public int DeleteMainData(String strType, String strShopID,
		...
		try {
			if (strType.equals("0")) {
				strSql = "delete from CheckMain where ShopID=? and CheckID=?";
				db.execSQL(strSql, new Object[] { strShopID, strCheckID });
				dbLocal.execSQL(strSql, new Object[] { strShopID, strCheckID });
			} else {
				strSql = "delete from CheckMain where ShopID=? and CheckID=? and PositionID=?";
				db.execSQL(strSql, new Object[] { strShopID, strCheckID,
						strPositionID });
				dbLocal.execSQL(strSql, new Object[] { strShopID, strCheckID,
						strPositionID });
	...
	public int DeleteAllMainData(String strShopID) {
		int iRet = 1;
		String strSql = "delete from CheckMain where ShopID=?";
		db.beginTransaction();
		dbLocal.beginTransaction();
		try {
	...
	public GdInfo GetDataByCode(String strGdCode) {
		String strSql = "Select GdBarCode,GdCode,GdArtNO,GdStyle,GdName,GdColorID," + "GdColorName,GdSizeID,GdSizeName,Property1,Property2,GdStock,GdPrice from GdInfo where GdCode=?";
		Cursor cursor = null;
		GdInfo gdInfo = null;
		db.beginTransaction();
		try {
	...
	public BarSplitLenSet GetBarSplitLenSetInUsage() {
		...
		String strSql = "Select Item,ArtNO,Style,Color,Size,ArtNOSerial,StyleSerial,ColorSerial,SizeSerial,"
				+ "Separator1,Separator2 From BarSplitLenSet where InUsage=1";
		Cursor cursor = null;
		db.beginTransaction();
		try {
			cursor = db.rawQuery(strSql, null);
	...
	public BarSplitLenSet GetSpecifiedItem(String iItem) {
		...
		String strSql = "Select Item,ArtNO,Style,Color,Size,ArtNOSerial,StyleSerial,ColorSerial,SizeSerial,Separator1,"
				+ "Separator2 From BarSplitLenSet where Item=?";
		Cursor cursor = null;
		db.beginTransaction();
		try {
	...
	public int UpdateBarSplitLenSetInUsage(BarSplitLenSet barSplitLenSet) {
		int iRet = 1;
		String strSql = "Update BarSplitLenSet set InUsage = 0";
		db.beginTransaction();
		try {
			db.execSQL(strSql);
			strSql = "Update BarSplitLenSet "
	...
	// public int ReplaceSDFBySavedConfigSDF()
	// {
	// int iRet = 1;
	// try
	// {
	...
	// if (strFullPath.Equals(DBAccess.DBAccess.DALDir.AbsolutePath) == false)
	// return 0;
	//
	// //string strFile1 = string.Format(@"{0}\{1}",
	// DBAccess.DBAccess.DALDir.AbsolutePath,
	// DBAccess.DBAccess.DALDir.SqlceDatabaseName);
	// string strFile2 = string.Format(@"{0}\{1}",
	// DBAccess.DBAccess.DALIndir.AbsolutePath,
	// DBAccess.DBAccess.DALIndir.SqlceDatabaseName);
	// string strFile3 = string.Format(@"{0}\BackDB\PD.sdf", strFullPath);
	//
	// if (System.IO.File.Exists(strFile3) == false)
	// {
	// System.IO.File.Copy(string.Format(@"{0}\PD.sdf", strFullPath), strFile3);
	// //true,OverWriten
	// }
	// else
	// {
	// System.IO.File.Delete(strFile3);
	// System.IO.File.Copy(string.Format(@"{0}\PD.sdf", strFullPath), strFile3);
	// }
	// SupoinInven.Config.ConfigLogic.Instance.Param.ReplaceIndirDB = "1";
	// SupoinInven.Config.ConfigLogic.Instance.SaveConfig();
	// }
	// catch (System.Exception ex)
	// {
	// Log.WriteLog("TransferFromDirToIndir Error:", ex.Message);
	// iRet = 0;
	// }
	// return iRet;
	// }
	//
	// /// <summary>
	// /// 恢复启用唯一码后写入的数据表。在这里无论程序是否启用了唯一码，都执行该动作，20131014
	// /// </summary>
	// /// <returns></returns>
	// public int RestoreCheckDetailSingleFromIndirtoDir()
	// {
	...
	// CheckTime,GdName,GdColorName,GdSizeName,Property1,Property2,GdStock,GdPrice,ReservedMg,GdBarSingle
	// from CheckDetailSingle";
	//
	// string strTableDetailSingle = @"Select
	// ShopID,CheckID,PositionID,GdBar,CheckNum,GdArtNO,GdStyle,GdColorID,GdSizeID,
	// CheckTime,GdName,GdColorName,GdSizeName,Property1,Property2,GdStock,GdPrice,ReservedMg,GdBarSingle
	// from CheckDetailSingle";
	//
	// DBAccess.DBAccess.DALIndir.ExecuteReader(sql,
	// delegate(System.Data.IDataReader dr)
	// {
	// DBAccess.DBAccess.DALDir.BulkCopy(strTableDetailSingle, false,
	// delegate(System.Data.SqlServerCe.SqlCeResultSet rsSet,
	// System.Data.SqlServerCe.SqlCeUpdatableRecord rd)
	// {
	// while (dr.Read())
	// {
	// rd.SetValue(0, dr.IsDBNull(0) ? "" : dr.GetString(0));
	// rd.SetValue(1, dr.IsDBNull(1) ? "" : dr.GetString(1));
	// rd.SetValue(2, dr.IsDBNull(2) ? "" : dr.GetString(2));
	// rd.SetValue(3, dr.IsDBNull(3) ? "" : dr.GetString(3));
	// rd.SetValue(4, dr.IsDBNull(4) ? 0 : dr[4]);
	// rd.SetValue(5, dr.IsDBNull(5) ? "" : dr.GetString(5));
	// rd.SetValue(6, dr.IsDBNull(6) ? "" : dr.GetString(6));
	// rd.SetValue(7, dr.IsDBNull(7) ? "" : dr.GetString(7));
	// rd.SetValue(8, dr.IsDBNull(8) ? "" : dr.GetString(8));
	//
	// rd.SetValue(9, dr.IsDBNull(9) ? DateTime.Now : dr.GetDateTime(9));
	// rd.SetValue(10, dr.IsDBNull(10) ? "" : dr.GetString(10));
	// rd.SetValue(11, dr.IsDBNull(11) ? "" : dr.GetString(11));
	// rd.SetValue(12, dr.IsDBNull(12) ? "" : dr.GetString(12));
	// rd.SetValue(13, dr.IsDBNull(13) ? "" : dr.GetString(13));
	// rd.SetValue(14, dr.IsDBNull(14) ? "" : dr.GetString(14));
	// rd.SetValue(15, dr.IsDBNull(15) ? 0 : dr[15]);
	// rd.SetValue(16, dr.IsDBNull(16) ? 0 :
	// Convert.ToDecimal(dr.GetValue(16).ToString()));
	// rd.SetValue(17, dr.IsDBNull(17) ? "" : dr.GetString(17));
	// rd.SetValue(18, dr.IsDBNull(18) ? "" : dr.GetString(18));
	//
	// rsSet.Insert(rd);
	// }
	// });
	...
	// {
	// iRet = 0;
	// Log.WriteLog("RestoreCheckDetailSingle Error:", ex.Message);
	// throw ex;
	// }
	//
	// return iRet;
	...
	// /// </summary>
	// /// <returns></returns>
	// public int TransferFromIndirToDir()
	// {
	// string strSql = string.Empty;
	// int iRet = 1;
	//
	...
	// {
	// string sql =
	// "Select distinct shopID,CheckID,PositionID from checkDetail";
	// //string strTableMain = "CheckMain";
	// string strTableDetail = "checkDetail";
	...
	// {
	// strTmp =
	// string.Format("insert into CheckMain(shopID,CheckID,PositionID) values('{0}','{1}','{2}')",
	// dr.IsDBNull(0) ? "" : dr.GetString(0),
	// dr.IsDBNull(1) ? "" : dr.GetString(1),
	// dr.IsDBNull(2) ? "" : dr.GetString(2));
	// DBAccess.DBAccess.DALDir.ExecuteNonQuery(strTmp, true);
	// }
	// DBAccess.DBAccess.DALDir.Save();
	// });
	//
	// //sql =
	// "Select ShopID,CheckID,PositionID,GdBar,Sum(CheckNum),GdArtNO,GdStyle,GdColorID,GdSizeID from CheckDetail Group by ShopID,CheckID,PositionID,GdBar,GdArtNO,GdStyle,GdColorID,GdSizeID";
	// //strTableDetail =
	// "Select ShopID,CheckID,PositionID,GdBar,CheckNum,GdArtNO,GdStyle,GdColorID,GdSizeID from CheckDetail";
	//
	// sql = @"Select ShopID,CheckID,PositionID,GdBar,Sum(CheckNum)
	// Num,GdArtNO,GdStyle,GdColorID,GdSizeID,
	// Min(checkTime) CheckTime,
	// GdName,GdColorName,GdSizeName,Property1,Property2,GdStock,GdPrice,ReservedMg
	// from CheckDetail Group by
	// ShopID,CheckID,PositionID,GdBar,GdArtNO,GdStyle,GdColorID,GdSizeID,
	// GdName,GdColorName,GdSizeName,Property1,Property2,GdStock,GdPrice,ReservedMg";
	//
	// strTableDetail = @"Select
	// ShopID,CheckID,PositionID,GdBar,CheckNum,GdArtNO,GdStyle,GdColorID,GdSizeID,
	// CheckTime,
	// GdName,GdColorName,GdSizeName,Property1,Property2,GdStock,GdPrice,ReservedMg
	// from CheckDetail";
	//
	// DBAccess.DBAccess.DALIndir.ExecuteReader(sql,delegate(System.Data.IDataReader
	// dr)
	// {
	// DBAccess.DBAccess.DALDir.BulkCopy(strTableDetail, false,
	// delegate(System.Data.SqlServerCe.SqlCeResultSet rsSet,
	// System.Data.SqlServerCe.SqlCeUpdatableRecord rd)
	// {
	// while (dr.Read())
	...
	// });
	//
	// RestoreCheckDetailSingleFromIndirtoDir();
	// }
	// catch (System.Exception ex)
	// {
	// iRet = 0;
	// Log.WriteLog("TransferFromIndirToDir Error:", ex.Message);
	// }
	//
	// return iRet;
	// }
	//
	// public int RepairDB(SalesPoint.PD.Data.IDALayer dal)
	// {
	// try
	// {
	// SqlCeEngine ce = new SqlCeEngine();
	// ce.Repair(dal.ConnectionString, RepairOption.RecoverCorruptedRows);
	// }
	// catch (System.Exception ex)
	// {
	// Log.WriteLog("RepairDB Error:", ex.Message);
	// return 0;
	// }
	// return 1;
	// }
	//
	// public int TransferFromDirToIndir()
	// {
	// int iRet = 1;
	// try
	// {
	...
	// DBAccess.DBAccess.DALIndir.SqlceDatabaseName);
	//
	// if (System.IO.File.Exists(strFile2) == false)
	// {
	// System.IO.File.Copy(string.Format(@"{0}\PD.sdf", strFullPath), strFile2);
	// //true,OverWriten
	// }
	// else
	// {
	// System.IO.File.Delete(strFile2);
	// System.IO.File.Copy(string.Format(@"{0}\PD.sdf", strFullPath), strFile2);
	// }
	// }
	// catch (System.Exception ex)
	...
	public boolean InsertCheckData(List<CheckDetail> checkData) {
		boolean isok = false;
		String sql = "insert into CheckDetail (ShopID,CheckID,PositionID,GdBar,CheckNum,GdArtNO,GdStyle,GdColorID,GdSizeID,"
				+ "GdName,GdColorName,GdSizeName,Property1,Property2,GdStock,GdPrice,ReservedMg,CheckTime)"
				+ " values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
	...
	public List<GdInfo> getCheckInfoList(List<String> rfidList,String inOutCode) {
		...
		if(inOutCode.equals("0")){//内部码
			strSql = "Select GdBarCode,GdCode,GdArtNO,GdStyle,GdName,GdColorID,GdColorName,GdSizeID,GdSizeName,Property1,Property2,GdStock,GdPrice from GdInfo where GdCode = ?";
		}else{//外部码
			strSql = "Select GdBarCode,GdCode,GdArtNO,GdStyle,GdName,GdColorID,GdColorName,GdSizeID,GdSizeName,Property1,Property2,GdStock,GdPrice from GdInfo where GdBarCode = ?";
		}
		Cursor cursor = null;
	...
	/**验证---插入缓存表*/
	public synchronized int InsertCheckDetailTempRFIDVer(List<GdInfo> rfidList,String strShopID, String strCheckID, String strPositionID,String inOutCode) {
		int iRet = 0;
		String strSql = "insert into CheckDetailTemTable (ShopID,CheckID,PositionID,GdBar,CheckNum,GdArtNO,GdStyle,GdColorID,GdSizeID,"
				+ "GdName,GdColorName,GdSizeName,Property1,Property2,GdStock,GdPrice,ReservedMg,CheckTime)"
				+ " values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
		String deleteSqlString = "delete from CheckDetailTemTable";
	...
	/**不验证---插入缓存表*/
	public synchronized int InsertCheckDetailTempRFID(List<String> rfidList,String strShopID, String strCheckID, String strPositionID) {
		int iRet = 0;
		String strSql = "insert into CheckDetailTemTable (ShopID,CheckID,PositionID,GdBar,CheckNum,GdArtNO,GdStyle,GdColorID,GdSizeID,"
				+ "GdName,GdColorName,GdSizeName,Property1,Property2,GdStock,GdPrice,ReservedMg,CheckTime)"
				+ " values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
	...
	public synchronized int InsertCheckDetailRFID() {
		...
		String strSql = "insert into CheckDetail(ShopID,CheckID,PositionID,GdBar,CheckNum,GdArtNO,GdStyle,GdColorID,GdSizeID,"
				...
				+ "GdName,GdColorName,GdSizeName,Property1,Property2,GdStock,GdPrice,ReservedMg,CheckTime from CheckDetailTemTable"
				+" t1 where not exists(select 1 from CheckDetail t2 where t1.GdBar=t2.GdBar and t1.ShopID=t2.ShopID and t1.CheckID=t2.CheckID and t1.PositionID=t2.PositionID)";
		db.beginTransaction();
		dbLocal.beginTransaction();
		try {
	...
	public boolean checkRFIDCodeByCheckDetail(String strBar, String strShopID,
		...
		Cursor cursor = null;
		String strSql = "select GdBar from CheckDetail where ShopID=? and CheckID=? and PositionID=? and GdBar=?";
		db.beginTransaction();
		try {
			cursor = db.rawQuery(strSql, new String[] { strShopID, strCheckID,
