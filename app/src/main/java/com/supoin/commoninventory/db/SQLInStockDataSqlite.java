...
public class SQLInStockDataSqlite extends SQLDataSqlite {

	protected StockDataOpenHelper helper = null;
	protected LocalStockDataOpenHelper helperLocal = null;

	public SQLInStockDataSqlite() {
		super();
	}

	public SQLInStockDataSqlite(Context context) {
		this.context = context;
		helper = StockDataOpenHelper.getInstance(context);
	}

	public SQLInStockDataSqlite(Context context, boolean isWrite) {
		this.context = context;
		helper = StockDataOpenHelper.getInstance(context);
		if (isWrite) {
	...
	public void saveExportSet(ExportSet exportSet) {
		String strSql = "delete from InStockExportSetInfo";
		db.beginTransaction();
		try {
			db.execSQL(strSql);
			strSql = "insert into InStockExportSetInfo values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
	...
	public List<ExportSet> getExportSetList() {
		...
		Cursor cursor = null;
		String strSql = "select * from InStockExportSetInfo";
		db.beginTransaction();
		try {
			cursor = db.rawQuery(strSql, null);
			while (cursor.moveToNext()) {
			...
			e.printStackTrace();
			Utility.deBug(" getInStockExportSetInfo Method Error", e.toString());
		} finally {
			db.endTransaction();
			cursor.close();
		}
	...
	public int DeleteDetailData(String strType, CheckDetail checkDetail) {
		...
		try {
			String strSql = "";
			if (strType.equals("0")) {
				strSql = "delete from CheckInDetail where ShopID=? and CheckID=?";
				db.execSQL(strSql, new Object[] { checkDetail.strShopID,
						checkDetail.strCheckID });
			} else {
				strSql = "delete from CheckInDetail where ShopID=? and CheckID=? and PositionID=?";
				db.execSQL(strSql, new Object[] { checkDetail.strShopID,
						checkDetail.strCheckID, checkDetail.strPositionID });
			}
		} catch (Exception ex) {
			iRet = 0;
			Utility.deBug("CheckInDetail DeleteDetailData Error:",
					ex.getMessage());
		}
		return iRet;
	}
	...
	public List<String[]> GetExportDatasSplitBarcode(String strExportType,
		...
		try {
			...
			if (strExportType.equals("0")) {
				strSql = "select " + sName + " From CheckInDetail"
			...
			Utility.deBug("CheckInDetail GetExportCustom Method Error",
					ex.getMessage());
		} finally {
			db.endTransaction();
			cursor.close();
		...
		try {
			...
			if (strExportType.equals("0")) {
				strSql = "select ShopID," + buffer
						+ "CheckNum,CheckTime From CheckInDetail"
	...
	public float GetSpeTotalQty(String strExportType, String strShopID,
		...
		try {
			...
			{
				strSql = "select CheckNum From CheckInDetail where ShopID=? And CheckID=?";
				cursor = db.rawQuery(strSql, new String[] { strShopID,
						strCheckID, strPositionID });
				while (cursor.moveToNext()) {
			...
			{
				strSql = "select CheckNum From CheckInDetail where ShopID=? And CheckID=? And PositionID=?";
				cursor = db.rawQuery(strSql, new String[] { strShopID,
						strCheckID, strPositionID });
				while (cursor.moveToNext()) {
					float checkNum = cursor.getFloat(cursor
			...
			{
				strSql = "select CheckNum from CheckInDetail where ShopID=?";
				cursor = db.rawQuery(strSql, new String[] { strShopID });
				while (cursor.moveToNext()) {
					float checkNum = cursor.getFloat(cursor
							.getColumnIndex("CheckNum"));
			...
			Utility.deBug("CheckInDetail ExistSpecifyBar Method Error",
					ex.getMessage());
			// dTotal = -1M;
		} finally {
			db.endTransaction();
	...
	public int DeleteSingleData(String strType, String strCheckID,
		...
		try {
			...
			String strSqlSingle = "";
			if (strType.equals("0")) {
				strSqlMain = "delete from CheckInMain where ShopID=? and CheckID=?";
				strSqlDetail = "delete from CheckInDetail where ShopID=? and CheckID=?";
				strSqlSingle = "delete from InStockCheckDetailSingle where ShopID=? and CheckID=?";
				...
				strSqlMain = "delete from CheckInMain where ShopID=? and CheckID=? and PositionID=?";
				strSqlDetail = "delete from CheckInDetail where ShopID=? and CheckID=? and PositionID=?";
				strSqlSingle = "delete from InStockCheckDetailSingle where ShopID=? and CheckID=? and PositionID=?";
				db.execSQL(strSqlMain, new Object[] { strShopID, strCheckID,
						strPositionID });
				db.execSQL(strSqlDetail, new Object[] { strShopID, strCheckID,
						strPositionID });
	...
	public int DeleteAllDetailData(String shopID) {
		int iRet = 1;
		String strSql = "delete from CheckInDetail where ShopID=?";
		try {
			db.execSQL(strSql, new Object[] { shopID });
			dbLocal.execSQL(strSql, new Object[] { shopID });
		} catch (Exception ex) {
			iRet = 0;
			Utility.deBug("CheckInDetail DeleteAllDetailData Error:",
					ex.getMessage());
		}
		return iRet;
	}
	...
	public int DeleteAllCheckData(String strShopID) {
		int iRet = 1;
		String strSqlMain = "delete from CheckInMain where ShopID=?";
		String strSqlDetail = "delete from CheckInDetail where ShopID=?";
		String strSqlDetailSingle = "delete from InStockCheckDetailSingle where ShopID=?";
		try {
	...
	public int DeleteAllShops() {
		int iRet = 1;
		String strSqlMain = "delete from CheckInMain";
		String strSqlDetail = "delete from CheckInDetail";
		String strSqlDetailSingle = "delete from InStockCheckDetailSingle";
		try {
	...
	public int InsertCheckDetailBySpecial(CheckDetail checkDetail,
		...
		int iRet = 0;
		String strSql = "insert into CheckInDetail (ShopID,CheckID,PositionID,GdBar,CheckNum,GdArtNO,GdStyle,GdColorID,GdSizeID,"
		...
		try {
			...
			if (isInputNum.equals("0")) {
				...
				if (i != 1) {
					...
					if (j != 1) {
						...
						strSql = "Update CheckInDetail Set CheckNum=? where ShopID=? And CheckID=? and PositionID=? And GdBar=?";
						db.execSQL(strSql,
			...
			if (usingBarCode.equals("1")) {
				...
				String strSqlSingle = "Insert Into InStockCheckDetailSingle(ShopID,CheckID,PositionID,"
			...
			iRet = 0;
			Utility.deBug("CheckInDetail SqlInsert Method Error",
					ex.getMessage());
	...
	public int GetOriginalNum(String strShopID, String strCheckID,
		...
		int dAlreadyQty = 0;
		String strSql = "select CheckNum From CheckInDetail where ShopID=? And CheckID=? and PositionID=? and GdBar=?";
		Cursor cursor = null;
		db.beginTransaction();
		try {
			...
			Utility.deBug("CheckInDetail GetOriginalNum Method Error",
					ex.getMessage());
		} finally {
			db.endTransaction();
			cursor.close();
	...
	public List<ScanOrderInfo> GetRecordsScanOrder(String strShopID,
		...
		String strSql = "select ID,GdBar,CheckNum from CheckInDetail where ShopID =?"
		...
		try {
			...
			Utility.deBug("CheckInDetail GetRecordsScanOrder Method Error",
					ex.getMessage());
		} finally {
			dbLocal.endTransaction();
			cursor.close();
	...
	public List<StaPositionSummary> GetStaPositionSummary(String strShopID,
		...
		String strSql = "select PostionID from CheckInDetail where shopID=? and CheckID=? order by PositionID";
		Cursor cursor = null;
		float sums = 0;
		db.beginTransaction();
		try {
	...
	public List<StaCheckNODetail> GetStaCheckNODetail(String strShopID,
		...
		Cursor cursor = null;
		String strSql = "select ID,PositionID,GdBar,CheckNum from CheckInDetail where ShopID=? and CheckID=? order by PositionID,GdBar";
		db.beginTransaction();
		try {
			cursor = db
					.rawQuery(strSql, new String[] { strShopID, strCheckID });
	...
	public List<CheckListEntity> GetStaCheckPositionDetail(String strShopID,
		...
		Cursor cursor = null;
		String strSql = "select GdBar from CheckInDetail where ShopID=? and CheckID=? and PositionID=? order by CheckTime desc";
		db.beginTransaction();
		try {
			cursor = db.rawQuery(strSql, new String[] { strShopID, strCheckID,
					strPositionID });
			while (cursor.moveToNext()) {
				...
				Cursor cursor1 = db
						.rawQuery(
								"select CheckNum from CheckInDetail where GdBar=? and ShopID=? and CheckID=? and PositionID=?",
	...
	public List<StaCheckDetail> GetStaCheckDetail(String strShopID,
		...
		Cursor cursor = null;
		String strSql = "Select ID,PositionID from CheckInDetail where ShopID=? and CheckID=? and GdBar=? order by PositionID";
		db.beginTransaction();
		try {
			cursor = db.rawQuery(strSql, new String[] { strShopID, strCheckID,
					strGdBar });
			while (cursor.moveToNext()) {
				...
				Cursor cursor1 = db
						.rawQuery(
								"select CheckNum from CheckInDetail where PositionID=?",
	...
	public int ExistSpecifyBar01(String strShopID, String strCheckID,
		...
		int iRet = 0;
		Cursor cursor = null;
		String strSql = "select * from InStockCheckDetailSingle where ShopID=? And CheckID=? and GdBarSingle=? and CheckNum > 0";
		db.beginTransaction();
		try {
			cursor = db.rawQuery(strSql, new String[] { strShopID, strCheckID,
					strSingleGdBar });
	...
	public int checkUniqueCodeFromCheckDetail(String strShopID,
		...
		int iRet = 0;
		Cursor cursor = null;
		String strSql = "select GdArtNO from CheckInDetail where GdBar=? and ShopID = ?  and CheckID = ?";
		db.beginTransaction();
		try {
			cursor = db.rawQuery(strSql, new String[] { strSingleGdBar,
	...
	public int ExistSpecifyBar02(String strShopID, String strCheckID,
		...
		int iRet = 0;
		Cursor cursor = null;
		String strSql = "Select * From InStockCheckDetailSingle where ShopID=? And CheckID=? and PositionID=? and GdBarSingle=? And CheckNum > 0";
		db.beginTransaction();
		try {
			cursor = db.rawQuery(strSql, new String[] { strShopID, strCheckID,
	...
	public int SaveCheckDetail(int dOldNum, CheckDetail checkDetail,
		...
		try {
			if (checkDetail.dCheckNum == 0) {
				strSql = "delete from CheckInDetail where ShopID=? And CheckID=? and PositionID=? And GdBar=?";
				...
				String sqlString = "select count(*) from CheckInDetail where ShopID=? And CheckID=? and PositionID=? and GdBar=?";
				...
				while (condition.moveToNext()) {
					...
					if (condition.getInt(condition.getColumnIndex("count(*)")) == 0) {
						String test = "Insert Into CheckInDetail(ShopID,CheckID,PositionID,GdBar,CheckNum,GdArtNO,GdStyle,GdColorID,GdSizeID,GdName,GdColorName,GdSizeName,Property1,Property2,GdStock,GdPrice,ReservedMg,CheckTime)"
								+ "values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
						db.execSQL(
								test,
								new Object[] { checkDetail.strShopID,
						...
						strSql = "Update CheckInDetail Set CheckNum=? , CheckTime = ? where ShopID=? And CheckID=? and PositionID=? And GdBar=?";
						db.execSQL(strSql,
			...
			if (checkDetail.dCheckNum == 0) {
				...
				strSql = "Insert Into CheckInDetail(ShopID,CheckID,PositionID,GdBar,CheckNum,GdArtNO,GdStyle,GdColorID,GdSizeID,"
						+ "GdName,GdColorName,GdSizeName,Property1,Property2,GdStock,GdPrice, ReservedMg,CheckTime) "
						+ "values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
				dbLocal.execSQL(strSql, new Object[] { checkDetail.strShopID,
						checkDetail.strCheckID, checkDetail.strPositionID,
			...
			if (sp.getString(ConfigEntity.UsingBarCodeKey,
				...
				if (checkDetail.dCheckNum < dOldNum) {
					String strSql1 = "select ID from InStockCheckDetailSingle where ShopID=? And CheckID=? and PositionID=? And GdBarSingle=?";
					cursor = db.rawQuery(strSql1, new String[] {
							checkDetail.strShopID, checkDetail.strCheckID,
							checkDetail.strPositionID,
							checkDetail.strSingleGdBar });
					while (cursor.moveToNext()) {
						strSqlSingle = "delete from InStockCheckDetailSingle where ID =?";
					...
					strSqlSingle = "Insert Into InStockCheckDetailSingle(ShopID,checkID,PositionID,GdBar,CheckNum,"
			...
			iRet = 0;
			Utility.deBug("CheckInDetail SqlCheckAndInsert Method Error",
					ex.getMessage());
		} finally {
			db.endTransaction();
			dbLocal.endTransaction();
		...
		try {
			if (false)// GlobalRunConfig.GetInstance().iScanDisplayWay == 0)
			{
				strSql = "Select a.GdBar,a.CheckNum,a.GdArtNO,"
						...
						+ "a.Property1,a.Property2,a.GdStock,a.GdPrice,a.ID From InStockCheckDetailSingle a where a.ShopID=?"
						...
						ConfigEntity.InOutCode).equals("1")) // 内部码
				{
					strSql = "Select  a.GdBar,a.CheckNum,a.GdArtNO,a.GdStyle,a.GdName,a.GdColorID,"
							...
							+ "From CheckInDetail a where a.ShopID=?"
							+ "And a.CheckID=? and a.PositionID=? order by ID desc Limit "
							+ iRows;
				} else if (sp.getString(ConfigEntity.InOutCodeKey,
			...
			Utility.deBug("CheckInDetail GetSpecifiedQtyRecords Method Error",
					ex.getMessage());
		} finally {
			db.endTransaction();
			if (null != cursor) {
	...
	public float GetSpecifiedCheckSum(String strShopID, String strCheckID) {
		float dCheckSum = 0;
		Cursor cursor = null;
		String strSql = "Select CheckNum From CheckInDetail where ShopID=? And CheckID=?";
		db.beginTransaction();
		try {
			cursor = db
					.rawQuery(strSql, new String[] { strShopID, strCheckID });
			...
			Utility.deBug("CheckInDetail GetSpecifiedSum Method Error",
					ex.getMessage());
		} finally {
			db.endTransaction();
			cursor.close();
	...
	public int GetSpecifiedPositionSum(String strShopID, String strCheckID,
		...
		int dCheckSum = 0;
		Cursor cursor = null;
		String strSql = "Select CheckNum From CheckInDetail where ShopID=? And CheckID=? and PositionID=?";
		db.beginTransaction();
		try {
			cursor = db.rawQuery(strSql, new String[] { strShopID, strCheckID,
					strPositionID });
	...
	public int GetSpecifiedCheckIDSum(String strShopID, String strCheckID,
		...
		// dPositionSum = 0;
		Cursor cursor = null;
		String strSql = "Select sum(CheckNum) From CheckInDetail where ShopID=? And CheckID=?";
		db.beginTransaction();
		try {
	...
	public int GetSpecifiedPositionIdSum(String strShopID, String strCheckID,
		...
		// dPositionSum = 0;
		Cursor cursor = null;
		String strSql = "Select sum(CheckNum) From CheckInDetail where ShopID=? And PositionId=? And CheckId = ?";
		db.beginTransaction();
		try {
			cursor = db.rawQuery(strSql, new String[] { strShopID,
	...
	public int GetGoodsQty(String strShopID, String strCheckID,
		...
		try {
			...
			Utility.deBug("CheckInDetail GetGoodsQty Method Error",
					ex.getMessage());
		} finally {
			db.endTransaction();
			if (cursor1 != null) {
	...
	public CheckDetail GetSpeCheckRecord(String strShopID, String strCheckID,
		...
		String strSql = "select GdBar,CheckNum,GdArtNO,GdStyle,GdName,GdColorID,"
				...
				+ "From CheckInDetail where ShopID=?"
		...
		try {
			...
			Utility.deBug("CheckInDetail GetSpeCheckRecord Method Error:",
					ex.getMessage());
		} finally {
			db.endTransaction();
	...
	public int SaveCheckMain(String strShopID, String strCheckID,
		...
		int iRet = 0;
		String strSql = "insert into CheckInMain(ShopID,CheckID,PositionID,CheckTime) values(?,?,?,?)";
		db.beginTransaction();
		try {
			...
			iRet = 0;
			Utility.deBug("CheckInMain SqlInsert Error", ex.getMessage());
		} finally {
			db.endTransaction();
		}
		return iRet;
	...
	public MainSummaryInfo GetSummaryQty(String strShopID) {
		...
		String strSql = "select a.CheckID,a.PositionID,"
				+ " from CheckInMain a Left Join CheckInDetail b on a.ShopID=b.ShopID and a.CheckID=b.CheckID and a.PositionID=b.PositionID "
		...
		try {
			...
			while (cursor.moveToNext()) {
				...
				summaryInfo.PositionID = cursor.getString(cursor
						.getColumnIndex("PositionID"));
				Cursor cursor1 = db.rawQuery(
						"select CheckNum from CheckInDetail where CheckID=? and "
		...
		try {
			...
			while (cursor.moveToNext()) {
				...
				summaryInfo.PositionID = cursor.getString(cursor
						.getColumnIndex("PositionID"));
				Cursor cursor1 = db.rawQuery(
						"select CheckNum from CheckInDetail where ShopID=? and CheckID=? and "
			...
			Utility.deBug("CheckInMain GetSummaryList Error", ex.getMessage());
		} finally {
			db.endTransaction();
			if (cursor != null)
				cursor.close();
	...
	public List<CheckMain> GetMainData(String strType, String strShopID) {
		...
		Cursor cursor = null;
		if (strType.equals("0")) {
			strSql = "select distinct CheckID from CheckInMain where ShopID=?";
		} else {
			strSql = "select distinct CheckID,PositionID from CheckInMain where ShopID=?";
		}
		db.beginTransaction();
		try {
			cursor = db.rawQuery(strSql, new String[] { strShopID });
	...
	public List<String> GetSpeCheckIDPositionID(String strShopID,
		...
		String strSql = "select distinct PositionID from CheckInMain where ShopID=? and CheckID=?'";
		Cursor cursor = null;
		db.beginTransaction();
		try {
			cursor = db
	...
	public int DeleteMainData(String strType, String strShopID,
		...
		try {
			if (strType.equals("0")) {
				strSql = "delete from CheckInMain where ShopID=? and CheckID=?";
				db.execSQL(strSql, new Object[] { strShopID, strCheckID });
				dbLocal.execSQL(strSql, new Object[] { strShopID, strCheckID });
			} else {
				strSql = "delete from CheckInMain where ShopID=? and CheckID=? and PositionID=?";
				db.execSQL(strSql, new Object[] { strShopID, strCheckID,
						strPositionID });
				dbLocal.execSQL(strSql, new Object[] { strShopID, strCheckID,
						strPositionID });
	...
	public int DeleteAllMainData(String strShopID) {
		int iRet = 1;
		String strSql = "delete from CheckInMain where ShopID=?";
		db.beginTransaction();
		dbLocal.beginTransaction();
		try {
	...
	public boolean InsertCheckData(List<CheckDetail> checkData) {
		boolean isok = false;
		String sql = "insert into CheckInDetail (ShopID,CheckID,PositionID,GdBar,CheckNum,GdArtNO,GdStyle,GdColorID,GdSizeID,"
				+ "GdName,GdColorName,GdSizeName,Property1,Property2,GdStock,GdPrice,ReservedMg,CheckTime)"
				+ " values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
	...
	public synchronized int InsertCheckDetailTempRFIDVer(List<GdInfo> rfidList,
			...
			String inOutCode) {
		int iRet = 0;
		String strSql = "insert into InStockTemTable (ShopID,CheckID,PositionID,GdBar,CheckNum,GdArtNO,GdStyle,GdColorID,GdSizeID,"
				+ "GdName,GdColorName,GdSizeName,Property1,Property2,GdStock,GdPrice,ReservedMg,CheckTime)"
				+ " values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
		String deleteSqlString = "delete from InStockTemTable";
	...
	public synchronized int InsertCheckDetailRFID() {
		...
		String strSql = "insert into CheckInDetail(ShopID,CheckID,PositionID,GdBar,CheckNum,GdArtNO,GdStyle,GdColorID,GdSizeID,"
				...
				+ "GdName,GdColorName,GdSizeName,Property1,Property2,GdStock,GdPrice,ReservedMg,CheckTime from InStockTemTable"
				+ " t1 where not exists(select 1 from CheckInDetail t2 where t1.GdBar=t2.GdBar and t1.ShopID=t2.ShopID and t1.CheckID=t2.CheckID and t1.PositionID=t2.PositionID)";
		db.beginTransaction();
		dbLocal.beginTransaction();
		try {
	...
	public boolean checkRFIDCodeByCheckDetail(String strBar, String strShopID,
		...
		Cursor cursor = null;
		String strSql = "select GdBar from CheckInDetail where ShopID=? and CheckID=? and PositionID=? and GdBar=?";
		db.beginTransaction();
		try {
			cursor = db.rawQuery(strSql, new String[] { strShopID, strCheckID,
...
public void exeSql(List<ContentValues> list) {
		
	    try{
	    	   db.beginTransaction(); // 手动设置开始事务
			   for (ContentValues v : list) {
				 db.insert("GdInfo", "0", v);
			   }
...
////	        String [] args = null;
//	        
////	        db.execSQL(strSql, objValue);
////	        HashMap<String, Object> values =new HashMap<>();
////	        db.insertWithOnConflict("GdInfo", "null",
////	                a,  iColumnNum);
////	        db.insertOrThrow("GdInfo", "0", a);
////	        db.insert("GdInfo", "null", a);
//	            db.setTransactionSuccessful();	  	  	
	    }catch(Exception ex){
...
//	    try{
//	        dbLocal.setTransactionSuccessful();
//	        dbLocal.execSQL(strSql, objValue);
//	        dbLocal.endTransaction();
//	  		
//	    }catch(Exception ex){
//	    	
