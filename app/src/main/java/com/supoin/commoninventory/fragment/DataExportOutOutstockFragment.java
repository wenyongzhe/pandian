...
public class DataExportOutOutstockFragment extends Fragment implements OnClickListener {
...
//		}
	...
	protected void ExportDatas() {
		...
		// 导出前缀
		strExportPrefix = sp.getString(ConfigEntity.ExportOutPrefixKey, "");
		// 是否导出扫描顺序，0为导出，1为不导出
		strIsExportScanOrder = sp.getString(ConfigEntity.IsExportSOKey, "");
		// 导出修饰符项，与数量间以,分隔，数量是否要最后一位为1表示数据包括修饰符
		exportDecaIndex = sp.getString(ConfigEntity.ExportDecaOutIndexKey, "0,0")
				.split(",");
		...
		new AsyncTask<Void, Void, Object>() {
			ProgressDialog dialog = AlertUtil.showProgressDialog(
					getActivity(), AlertUtil.getString(R.string.exporting_outstock),
					new View.OnClickListener() {
