...
/***
 * 盘点导出
 * @date 2017-5-25 下午4:07:27 
 */
public class DataExportOutInventoryFragment extends Fragment implements OnClickListener {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		sqlStockDataSqlite = new SQLStockDataSqlite(getActivity());
		...
		strExportType = sp.getString(ConfigEntity.ExportTypeKey, "");
		// 获取表头
		strExportDatasTitle = sp.getString("ExportDatasTitle", "").split(",");
...
//		}
	...
	protected void ExportDatas() {
		...
		// 导出前缀
		strExportPrefix = sp.getString(ConfigEntity.ExportPrefixKey, "");
		// 是否导出扫描顺序，0为不导出，1为导出
		strIsExportScanOrder = sp.getString(ConfigEntity.IsExportSOKey, "");
		// 导出修饰符项，与数量间以,分隔，数量是否要最后一位为1表示数据包括修饰符
		exportDecaIndex = sp.getString(ConfigEntity.ExportDecaIndexKey, "0,0")
				.split(",");
		...
		new AsyncTask<Void, Void, Object>() {
			ProgressDialog dialog = AlertUtil.showProgressDialog(
					getActivity(), AlertUtil.getString(R.string.exporting_inventory),
					new View.OnClickListener() {
...
//								stringBuffer0 = stringBuffer0.deleteCharAt(stringBuffer0.length() - 1).deleteCharAt(stringBuffer0.length() - 1);
								stringBuffer0 = stringBuffer0.delete(stringBuffer0.length() - 2,stringBuffer0.length());//去掉最后的换行
...
//										stringBuffer0 = stringBuffer0.deleteCharAt(stringBuffer0.length() - 1).deleteCharAt(stringBuffer0.length() - 1);
										stringBuffer0 = stringBuffer0.delete(stringBuffer0.length() - 2,stringBuffer0.length());
										StringBuffer strFileName = new StringBuffer();
										strDateWay = getDate();
										strFileName.append(strExportPrefix
												+ "_" + strCheckID + "_"
