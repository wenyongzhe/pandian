...
public class ScanDisplaySettingFragment2 extends Fragment {
	...
	SQLStockDataSqlite sqlStockData;
	// 扫描显示设置Item
	private String[] strItems;
	private int num;
	private StringBuffer strBuffer;
	private String[] strItem;
	private String displayItemsKey;
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		sp = getActivity().getSharedPreferences("InvenConfig",getActivity().MODE_PRIVATE);
		 int type= getArguments().getInt("id");  
		 switch (type) {
		 case 1:
			 displayItemsKey = ConfigEntity.DisplayItemsKey;
			break;
		 case 2:
			 displayItemsKey = ConfigEntity.DisplayItemsInKey;
			break;
		 case 3:
			 displayItemsKey = ConfigEntity.DisplayItemsOutKey;
			break;
		}
		
		strItems = sp.getString(displayItemsKey, "").split(",");
...
//					AlertUtil.showAlert(getActivity(), AlertUtil.getString(R.string.display1),AlertUtil.getString(R.string.display2));
					...
					if (language.endsWith("en")){
						AlertUtil.showAlert(getActivity(), AlertUtil.getString(R.string.display1),"Cannot choose "+list.get(0)+" and "+list.get(1)+"at the same time，Please reset it.");

					}else{
						AlertUtil.showAlert(getActivity(), AlertUtil.getString(R.string.display1),"不能同时选择"+list.get(0)+"和"+list.get(1)+"，请重新设置");
				...
				if (num > 5) {
					...
					sp.edit().putString(displayItemsKey,strBuffer.toString()).commit();
