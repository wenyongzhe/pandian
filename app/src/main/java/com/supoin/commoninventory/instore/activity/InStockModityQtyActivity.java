...
public class InStockModityQtyActivity extends Activity implements OnClickListener {
...
//	public  TextView name;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		CustomTitleBar.getTitleBar(InStockModityQtyActivity.this,AlertUtil.getString(R.string.enter_quantity), false);
	}else {
		CustomTitleBar.getTitleBar(InStockModityQtyActivity.this, AlertUtil.getString(R.string.modify_quantity), false);
	}
		...
		sqlStockDataSqlite = new SQLInStockDataSqlite(InStockModityQtyActivity.this,true);
...
//		});
	...
	private void keyDownOk() {
		String strQty = et_qty.getText().toString().trim();
		if (strQty.length() < 1) {
			AlertUtil.showAlert(InStockModityQtyActivity.this,R.string.warning, R.string.quantity_not_null,
...
//			}
			...
			if (i < 0) {	//没有小数点，即为整数的情况
				Intent intent = new Intent(InStockModityQtyActivity.this,
				...
				if (arr.length > 1) {
					if (arr[1].length() > 0) {
						if ("1".equals(type)) {
							if (arr[1].length() > 1) {
								AlertUtil.showAlert(InStockModityQtyActivity.this,
...
//				}
			...
			AlertUtil.showAlert(InStockModityQtyActivity.this, R.string.dialog_title, R.string.quantity_error,
					R.string.ok, new View.OnClickListener() {
	...
	private void popupInputMethodWindow() {
		handler.postDelayed(new Runnable() {
			@Override
			public void run() {
				InputMethodManager imm = (InputMethodManager) getSystemService(InStockModityQtyActivity.INPUT_METHOD_SERVICE);
				// 显示软键盘
	...
	public void onClick(View v) {
		...
		case R.id.btn_cancl:
			InStockDetailsActivity.is_intercept = true;
			InputMethodManager inputMethodManager = (InputMethodManager) getSystemService(InStockModityQtyActivity.INPUT_METHOD_SERVICE);
			inputMethodManager.hideSoftInputFromWindow(InStockModityQtyActivity.this
			...
			InStockDetailsActivity.tv_checknum.setText(String.valueOf(sqlStockDataSqlite.GetSpecifiedCheckSum(InStockDetailsActivity.strShopID, InStockDetailsActivity.strCheckID)));
