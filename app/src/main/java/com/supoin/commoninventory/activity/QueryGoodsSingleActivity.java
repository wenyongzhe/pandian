...
public class QueryGoodsSingleActivity extends Activity implements
		OnClickListener {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		CustomTitleBar
				.getTitleBar(QueryGoodsSingleActivity.this, AlertUtil.getString(R.string.goods_query), false);
		setContentView(R.layout.activity_query_goods_single);
		...
		sqlStockDataSqlite = new SQLStockDataSqlite(
				QueryGoodsSingleActivity.this, true);
		scanOperate = new ScanOperate();
		scanOperate.onCreate(QueryGoodsSingleActivity.this, R.raw.scanok);
	...
	private void setData() {
		...
		arrDisplayItem = sp.getString(ConfigEntity.DisplayItemsKey,
				ConfigEntity.DisplayItems).split(",");
		// 显示选项的导入串
					...
					public boolean onEditorAction(TextView v, int actionId,
						...
						if (actionId == EditorInfo.IME_ACTION_DONE
							...
							if (strBar.trim().length() < 1) {
								AlertUtil.showAlert(
										QueryGoodsSingleActivity.this, R.string.dialog_title,
	...
	public int ExecuteCutReserveBar(String fullBar, boolean bWithPrompt) {
		...
		if (strLengthCutOrKeepBarCode.equals("1")) // 截取后位数
		{
			if (Integer.parseInt(strLengthCutOrKeepBarCodeNum) >= strBar
					.length()) {
				if (bWithPrompt) {
					AlertUtil.showAlert(QueryGoodsSingleActivity.this, R.string.warning,
	...
	private void KeyDownOK() {
		...
		if (strBarCodeAuth.equals("1")) {	//条码验证
			...
			if (gdInfo == null) {
				...
				contentList.add("");
				setDisplayContent();
				AlertUtil.showToast("无此条码", QueryGoodsSingleActivity.this);
				...
				if (qty != 0) {
					...
					contentList.add("");
					setDisplayContent();
					AlertUtil.showToast(AlertUtil.getString(R.string.barCode_not_exist), QueryGoodsSingleActivity.this);
					tv_qty.setText("0");
	...
	public void onClick(View v) {
		...
		case R.id.btn_ok:
			strBar = et_goodsBar.getText().toString().trim();
			if (strBar.trim().length() < 1) {
				AlertUtil.showAlert(QueryGoodsSingleActivity.this, R.string.dialog_title,
			...
			KeyDownOK();
			//关闭软键盘
			InputMethodManager inputMethodManager1 = (InputMethodManager) getSystemService(QueryGoodsSingleActivity.INPUT_METHOD_SERVICE);
		...
		case R.id.btn_cancl:
			GoodsDetailsActivity.is_intercept = true;
			//关闭软键盘
			InputMethodManager inputMethodManager = (InputMethodManager) getSystemService(QueryGoodsSingleActivity.INPUT_METHOD_SERVICE);
