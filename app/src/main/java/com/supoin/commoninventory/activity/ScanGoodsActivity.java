...
import com.supoin.commoninventory.adapter.ScanGoodsListAdapter;
...
/**
 * 扫描单据界面
 * @date 2017-5-26 下午6:12:32 
 *
 */
public class ScanGoodsActivity extends Activity implements OnItemClickListener {
	private EditText et_CheckID, et_PositionID;
	private Button btn_add_goods;
	private TextView empty_list_view,tv_bianhao,tv_huowei;
	private ListView mListView;
	private ScanOperate scanOperate;
	private Boolean is_canScan = true;
	private ScanGoodsListAdapter mScanGoodsListAdapter;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		CustomTitleBar.getTitleBar(ScanGoodsActivity.this, R.string.document_scan, false);
		setContentView(R.layout.activity_scan_goods);
		sqlStockDataSqlite = new SQLStockDataSqlite(ScanGoodsActivity.this,true);
		...
		if (mMainSummaryInfoList != null && mMainSummaryInfoList.size() > 0) {
			mScanGoodsListAdapter = new ScanGoodsListAdapter(
					ScanGoodsActivity.this, mMainSummaryInfoList);
			mListView.setAdapter(mScanGoodsListAdapter);
			// mListView.setSelection(mScanGoodsListAdapter.getCount() - 1);
		} else {
		...
		mListView.setOnItemClickListener(new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> arg0, View arg1,
				...
				mMainSummaryInfoList = sqlStockDataSqlite
				...
				strPositionID = mMainSummaryInfoList.get(position)
						.getPositionID();
				Intent intent = new Intent(ScanGoodsActivity.this,
		...
		btn_add_goods.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				strPositionID = et_PositionID.getText().toString().trim()
						.toUpperCase();
				if (TextUtils.isEmpty(strCheckID)) {
					AlertUtil.showAlert(ScanGoodsActivity.this, R.string.dialog_title, bianhao+AlertUtil.getString(R.string.cannot_empty),
							R.string.ok, new View.OnClickListener() {
	...
	private void initText() {
		...
		String language = getResources().getConfiguration().locale.getLanguage();
		 if (language.endsWith("en")){
	        ...
	        btn_add_goods.setText(AlertUtil.getString(R.string.newly_added)+importStrArrayList.get(1)+importStrArrayList.get(2));
	    }
	}
	...
	private Handler handler = new Handler() {
		...
		public void handleMessage(Message msg) {
			...
			if (msg.what == 0) {
				...
				strPositionID = et_PositionID.getText().toString().trim()
						.toUpperCase(); // 库位编号
				final List<MainSummaryInfo> tempSummaryList = sqlStockDataSqlite
						.GetSummaryList(strCheckID, strPositionID, strShopID);
				if (tempSummaryList != null && tempSummaryList.size() > 0) {
					//存在该盘点单据
					AlertUtil.showAlert(ScanGoodsActivity.this,
							R.string.dialog_title, R.string.document_exists,
							R.string.ok, new View.OnClickListener() {
								@Override
								public void onClick(View v) {
									AlertUtil.dismissDialog();
									//保存数据到数据库中
									sqlStockDataSqlite.SaveCheckMain(strShopID,
									...
									if (mMainSummaryInfoList.size() == 1) {
										...
										mListView
												.setAdapter(mScanGoodsListAdapter);
										empty_list_view
												.setVisibility(View.GONE);
										mListView.setVisibility(View.VISIBLE);
									} else {
										mScanGoodsListAdapter
...
//									finish();
					...
					//保存数据到数据库中
					sqlStockDataSqlite.SaveCheckMain(strShopID, strCheckID,
							strPositionID);
					mMainSummaryInfo.setCheckID(strCheckID);
					...
					if (mMainSummaryInfoList.size() == 1) {
						...
						mScanGoodsListAdapter.setList(mMainSummaryInfoList);
						mListView.setAdapter(mScanGoodsListAdapter);
					}
...
//					finish();
	...
	protected void onResume() {
		...
		scanOperate.onCreate(ScanGoodsActivity.this, R.raw.scanok);
