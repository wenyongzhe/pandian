...
public class OutStockDetailsActivity extends BaseActivity implements OnClickListener {
	...
	// 编号，货位tv_positionid，名称，颜色，尺码，价格，库存，大类，小类，颜色ID，尺码IstrPositionIDD，款号，货号
	...
	//textview 修改 	tv_huowei_xq   tv_huoweisl
	private TextView tv_bianhao_xq,tv_bianhaosl;
	//是否消减
	private CheckBox cbDecrea;
	private SQLOutStockDataSqlite sqlOutStockDataSqlite;
	...
	private boolean isBtn = false; //是否点击修改数量按钮
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		CustomTitleBar.getTitleBar(OutStockDetailsActivity.this, R.string.outstock_sacn, false);
		setContentView(R.layout.activity_out_stock_goods_details);
		sqlOutStockDataSqlite = new SQLOutStockDataSqlite(OutStockDetailsActivity.this,true);
		//初始化sp
		...
		scanOperate.onCreate(OutStockDetailsActivity.this, R.raw.scanok);
	...
	private Handler mHandler = new Handler() {
		...
		public void handleMessage(Message msg) {
			...
			case UHF1MESSAGE_TEXT:
				...
				if (tagLists.size() < 1)
					return;
				//String[] temp = code2.substring(1, code2.length() - 1).split(",");
				//Collections.addAll(rfidList, temp);
			...
			case 4:
				...
				mCheckDetailList = sqlOutStockDataSqlite.GetRFIDCheckPositionDetail(strShopID, strCheckID, strPositionID);
				if(mGoodsDetailsListAdapter!=null){
					if(mCheckDetailList.size()>0){
						mGoodsDetailsListAdapter.setList(mCheckDetailList);
					}
				}else{
					mGoodsDetailsListAdapter = new GoodsDetailsListAdapter(OutStockDetailsActivity.this, mCheckDetailList);
				}
				mListView.setAdapter(mGoodsDetailsListAdapter);
				mGoodsDetailsListAdapter.notifyDataSetChanged();
				tv_checknum.setText(String.valueOf(sqlOutStockDataSqlite.GetSpecifiedCheckIDSum(strShopID, strCheckID, strPositionID)));
				if(rfidDialog!=null){
	...
	protected void loadingData() {
		rfidDialog = AlertUtil.showNoButtonProgressDialog(
				OutStockDetailsActivity.this, AlertUtil.getString(R.string.operate_check));
		smallExecutor.execute(new Runnable() {
			...
			public void run() {
				if (barCodeAuth.equals("1")){//验证
					...
					checkInfoList = sqlOutStockDataSqlite.getCheckInfoList(rfidRightList,inOutCode);
					sqlOutStockDataSqlite.InsertCheckDetailTempRFIDVer(checkInfoList,strShopID,strCheckID,strPositionID,inOutCode);
				}else{
					sqlOutStockDataSqlite.InsertCheckDetailTempRFID(rfidList,strShopID,strCheckID,strPositionID);
				}
			
				sqlOutStockDataSqlite.InsertCheckDetailRFID();
...
//		tv_huoweisl.setText(importStrArrayList.get(2)+AlertUtil.getString(R.string.quantity));
	...
	private void LoadData() {
		new AsyncTask<Void, Void, Object>() {
			ProgressDialog dialog = AlertUtil.showNoButtonProgressDialog(
					OutStockDetailsActivity.this, AlertUtil.getString(R.string.loading_data));
			...
			protected Object doInBackground(Void... params) {
				try {
					/*if (bEnableRFID) {
						rfidOperate.onCreate(OutStockDetailsActivity.this, DrugConstants.stockOpw);
					...
					if(isRFID){
						checkDetailStrList = sqlOutStockDataSqlite.getGdInfoBarcodeList(inOutCode);
					}
					// 判断显示方式，待处理或不处理，待确定...
					listTmp = sqlOutStockDataSqlite.GetSpecifiedCounterRecords(
					...
					dCheckSum = sqlOutStockDataSqlite.GetSpecifiedCheckIDSum(
							strShopID, strCheckID, strPositionID);
					result = "true";
					return result;
				} catch (Exception e) {
			...
			protected void onPostExecute(Object result) {
				...
				if (!TextUtils.isEmpty(result.toString())) {
					if (result.toString().equals("true")) {
						AlertUtil.showToast(AlertUtil.getString(R.string.load_success), OutStockDetailsActivity.this);
						myHandler.sendEmptyMessage(1);

					} else {
						AlertUtil.showAlert(OutStockDetailsActivity.this, R.string.dialog_title,
	...
	private void setData() {
		...
		//获取集合
		mCheckDetailList.clear();
		mCheckDetailList = sqlOutStockDataSqlite.GetStaCheckPositionDetail(strShopID, strCheckID, strPositionID);
		...
		dCheckSum = sqlOutStockDataSqlite.GetSpecifiedCheckIDSum(strShopID,
				strCheckID, strPositionID);
		dPositionSum = sqlOutStockDataSqlite.GetSpecifiedPositionIdSum(strShopID,
				strCheckID, strPositionID);
//		tv_qty.setText(String.valueOf(dPositionSum));
		...
		mListView.setOnItemClickListener(new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> arg0, View arg1, int arg2,
				...
				CheckDetail cd = sqlOutStockDataSqlite.GetSpeCheckRecord(
						strShopID, strCheckID, strPositionID,
				...
				if (sp.getString(ConfigEntity.BarCodeAuthKey,
					...
					CheckDetail checkDetail = sqlOutStockDataSqlite
	...
	private void insertRFID(final String strBar) {
		smallExecutor.execute(new Runnable() {
			...
			public void run() {
				rfidList.clear();
				rfidList.add(strBar);
				boolean isExit = sqlOutStockDataSqlite.checkRFIDCodeByCheckDetail(strBar,strShopID,strCheckID,strPositionID);
				if(isExit){//已存在该标签
					AlertUtil.showToast(R.string.RFID_HAS_EXIST, OutStockDetailsActivity.this);
				}else{
					if (barCodeAuth.equals("1")){//验证
						checkInfoList = sqlOutStockDataSqlite.getCheckInfoList(rfidList,inOutCode);
						if(checkInfoList==null){//判断标签是否在基础资料中
							AlertUtil.showToast(R.string.RFID_NOT_EXIST, OutStockDetailsActivity.this);
							rfidList.clear();
							rfidDialog.dismiss();
							return;
						}
	...
	public void onClick(View v) {
		if (v == btn_modify) {
			if(!isRfidStart){
				if(mCheckDetailList.size()-1 < itemSelected){//bug1336，越界
					itemSelected = 0;
				}
				if (sp.getString(ConfigEntity.ModifyNumPWKey,ConfigEntity.ModifyNumPW).equals("1")) {
					...
					if(mCheckDetailList.size() > 0){
						isPassAct = true;
						Intent intent = new Intent(OutStockDetailsActivity.this,PasswordInputScanGoodActivity.class);
						Bundle bundle = new Bundle();
						bundle.putString("title", AlertUtil.getString(R.string.pass_admin));
						intent.putExtras(bundle);
						startActivityForResult(intent, 10);
					}else{
						AlertUtil.showAlert(OutStockDetailsActivity.this, AlertUtil.getString(R.string.dialog_title),AlertUtil.getString(R.string.no_goods_scan));
					}
	...
	private void openQueryGoods() {
		...
		Intent intent = new Intent(OutStockDetailsActivity.this,
				OutStockQueryGoodsSingleActivity.class);
		Bundle bundle = new Bundle();
 ...
 */
	private void openModifyQty(Boolean isSaved) {
		...
		if (mCheckDetailList.size() > 0) {
			...
			if (isSaved) {
				...
				intent.setClass(OutStockDetailsActivity.this,OutStockModityQtyActivity.class);
				Bundle bundle = new Bundle();
	...
	private void GetSystemSetting() {
		...
		arrDisplayItem = sp.getString(ConfigEntity.DisplayItemsOutKey,ConfigEntity.DisplayItemsOut).split(",");
...
//							ivScan.setImageResource(R.drawable.barcode);
	...
	private void KeyDownOK() {
		try {
			...
			ex.printStackTrace();
			AlertUtil.showAlert(OutStockDetailsActivity.this, R.string.warning,
			...
			if (sp.getString(ConfigEntity.ScanningShowModeKey,
					ConfigEntity.ScanningShowMode).equals("1")) {
				tv_checknum.setText(String.valueOf(sqlOutStockDataSqlite
						.GetSpecifiedCheckIDSum(strShopID, strCheckID,
								strPositionID)));
//				tv_qty.setText(String.valueOf(sqlOutStockDataSqlite
 ...
 */
	...
	private void BarCodeSqlitAndInsert(String code) {
		...
		if (barCodeCutSetting.equals("1")) {
			...
			sqlOutStockDataSqlite.InsertCheckDetail(mCheckDetail, sp.getString(
	...
	private int BarDicreaAdd(GdInfo cg, CheckDetail cd) {
		...
		// 从基础数据中查询是否有此条码
		flag = sqlOutStockDataSqlite.checkUniqueCode(strShopID, strCheckID,strGdBar);
		// 基础数据有此条码的时候
		if (flag != 0) {
			// 从CheckDetail里查找是否有当前条码
			iCounter = sqlOutStockDataSqlite.checkUniqueCodeFromCheckDetail(strShopID, strCheckID, cd.strGdBar);
			// 基础数据无此条码时
		} else {
			// 从CheckDetail里查找是否有当前条码
			iCounter = sqlOutStockDataSqlite.checkUniqueCodeFromCheckDetail(strShopID, strCheckID,cd.strSingleGdBar);
		}
		...
		int dOriginalNum = 0;
		if (flag != 0) {
			dOriginalNum = sqlOutStockDataSqlite.GetOriginalNum(strShopID,strCheckID, strPositionID, cd.strGdBar);
		} else {
			dOriginalNum = sqlOutStockDataSqlite.GetOriginalNum(strShopID,strCheckID, strPositionID, cd.strSingleGdBar);
		}
		checkDetail.dCheckNum = dOriginalNum - 1;

		sqlOutStockDataSqlite.SaveCheckDetail(dOriginalNum, checkDetail, flag,false, 0,isBtn);
		dCheckSum -= 1;
		dPositionSum -= 1;
		...
		if (mCheckDetailList.size() > 0) {
			...
			AlertUtil.showAlert(OutStockDetailsActivity.this, AlertUtil.getString(R.string.dialog_title), AlertUtil.getString(R.string.no_unique_barcode));
//			refreshListView();
		}
...
//		refreshListView();
	...
	private int BarDicreaNoAdd(GdInfo cg, CheckDetail cd) {
		...
		// 从基础数据中查询是否有此条码
		flag = sqlOutStockDataSqlite.checkUniqueCode(strShopID, strCheckID,cd.strGdBar);
		// 基础数据有此条码的时候
		if (flag != 0) {
			// 从CheckDetail里查找是否有当前条码
			iCounter = sqlOutStockDataSqlite.checkUniqueCodeFromCheckDetail(strShopID, strCheckID, cd.strGdBar);
	...
	public int JudgeConditions(GdInfo cg, final CheckDetail cd) {
		...
		if (strBar.length() < 1) {
			AlertUtil.showAlert(OutStockDetailsActivity.this, R.string.dialog_title, AlertUtil.getString(R.string.barCode_not_empty),
		...
		if (sp.getString(ConfigEntity.UsingBarCodeKey,ConfigEntity.UsingBarCode).equals("1") && !cbDecrea.isChecked()) {
			...
			iTmp = sqlOutStockDataSqlite.checkUniqueCodeFromCheckDetail(strShopID,strCheckID,cd.strGdBar);

			if (iTmp == 1) {
				AlertUtil.showAlert(OutStockDetailsActivity.this, R.string.dialog_title, R.string.barcode_repeat,
						R.string.ok, new View.OnClickListener() {
		...
		if (sp.getString(ConfigEntity.BarCodeAuthKey, ConfigEntity.BarCodeAuth).equals("1")) {
			...
			//内部码的属性
			gdInfoInnerCode = sqlOutStockDataSqlite.GetDataByBar(strGdBar);
			//外部码的属性
			gdInfoOutterCode = sqlOutStockDataSqlite.GetDataByCode(strGdBar);
			...
			if (sp.getString(ConfigEntity.InOutCodeKey, ConfigEntity.InOutCode).equals("0")) {
				...
				checkFlag = sqlOutStockDataSqlite.checkUniqueCode(strShopID,strCheckID, cd.strGdBar);
				checkBarFromCheckDetail = sqlOutStockDataSqlite.checkUniqueCodeFromCheckDetail(strShopID, strCheckID,cd.strGdBar);
				...
				if (checkFlag != 1) {
					...
					if (checkBarFromCheckDetail != 1) {
						...
						if (cbDecrea.isChecked()) {
							...
							AlertUtil.showAlert(OutStockDetailsActivity.this,AlertUtil.getString(R.string.dialog_title),AlertUtil.getString(R.string.barcode_repeat));
							return 0;
						}
					}
				}
			...
			if (sp.getString(ConfigEntity.UsingBarCodeKey,ConfigEntity.UsingBarCode).equals("0")) {
				...
				if (checkFlag < 1) {
					if (checkBarFromCheckDetail != 1||(gdInfoInnerCode==null&&gdInfoOutterCode==null)) {
						AlertUtil.showAlert(OutStockDetailsActivity.this,
								...
								R.string.ok, new OnClickListener() {
									...
									public void onClick(View v) {
										...
										if (sp.getString(ConfigEntity.IsPutInNumKey,ConfigEntity.IsPutInNum).equals("0")) {
											...
											sqlOutStockDataSqlite.InsertCheckDetailBySpecial(cd,sp.getString(ConfigEntity.UsingBarCodeKey,ConfigEntity.UsingBarCode),sp.getString(ConfigEntity.BarCodeAuthKey,ConfigEntity.BarCodeAuth),tempStatus,sp.getString(ConfigEntity.IsPutInNumKey,ConfigEntity.IsPutInNum),1);
											// 0是累加
											if (sp.getString(ConfigEntity.ScanningShowModeKey,ConfigEntity.ScanningShowMode).equals("0")) {
												refreshListView();
												// 1是逐行
												...
												if (mGoodsDetailsListAdapter == null) {
													mGoodsDetailsListAdapter = new GoodsDetailsListAdapter(
															OutStockDetailsActivity.this, mCheckDetailList);
												}
												mGoodsDetailsListAdapter.setList(mCheckDetailList);
												mListView.setAdapter(mGoodsDetailsListAdapter);
												tv_checknum.setText(String.valueOf(sqlOutStockDataSqlite.GetSpecifiedCheckIDSum(strShopID, strCheckID, strPositionID)));
						...
						cd.strGdBar = strGdBar;
						if (sp.getString(ConfigEntity.ModifyNumPWKey,ConfigEntity.ModifyNumPW).equals("1")) {
							Intent intent = new Intent(OutStockDetailsActivity.this,PasswordInputActivity.class);
							Bundle bundle = new Bundle();
							bundle.putString("title", AlertUtil.getString(R.string.pass_admin));
							intent.putExtras(bundle);
							...
							AlertUtil.showAlert(
									...
									new OnClickListener() {
										...
										public void onClick(View v) {
											...
											sqlOutStockDataSqlite.InsertCheckDetail(cd,sp.getString(ConfigEntity.UsingBarCodeKey,ConfigEntity.UsingBarCode),sp.getString(ConfigEntity.BarCodeAuthKey,ConfigEntity.BarCodeAuth),tempStatus,sp.getString(ConfigEntity.IsPutInNumKey,ConfigEntity.IsPutInNum),1);
	...
	public int GdNumAdd(GdInfo cg, CheckDetail cd) {
		...
		if (sp.getString(ConfigEntity.InOutCodeKey, ConfigEntity.InOutCode).equals("0")) {
			...
			dOriginalNum = sqlOutStockDataSqlite.GetOriginalNum(strShopID,strCheckID, strPositionID, strGdBar);
		}

		if (dOriginalNum == 0) {
			i = sqlOutStockDataSqlite.InsertCheckDetail(cd, sp.getString(ConfigEntity.UsingBarCodeKey, ConfigEntity.UsingBarCode),sp.getString(ConfigEntity.BarCodeAuthKey,ConfigEntity.BarCodeAuth), tempStatus, sp.getString(ConfigEntity.IsPutInNumKey,ConfigEntity.IsPutInNum), 0);
			if (i == 1) {
				...
				AlertUtil.showAlert(OutStockDetailsActivity.this, R.string.error,
			...
			if (sp.getString(ConfigEntity.IsPutInNumKey,ConfigEntity.IsPutInNum).equals("0")) {
				...
				i = sqlOutStockDataSqlite.SaveCheckDetail(dOriginalNum, cd, 1,false, 0,isBtn);
 ...
 */
	public int GdNumNOAdd(GdInfo cg, CheckDetail cd) {
		...
		if (dOriginalNum == 0) {
			...
			//盘点数量
			cd.dCheckNum = cd.dCheckNum + dOriginalNum;
			i = sqlOutStockDataSqlite.SaveCheckDetail(dOriginalNum, cd, 1, false,
					0,isBtn);
			if (i != 1) {
				AlertUtil.showAlert(OutStockDetailsActivity.this, R.string.error,
						R.string.inventory_failed_check, R.string.ok,
 ...
 */
	private void saveCheckDetailQty() {
		...
		dOriginalNum = sqlOutStockDataSqlite.GetOriginalNum(strShopID, strCheckID,strPositionID, checkDetail.strGdBar);
		...
		if (sp.getString(ConfigEntity.BarCodeAuthKey, ConfigEntity.BarCodeAuth).equals("1")||sp.getString(ConfigEntity.BarCodeAuthKey, ConfigEntity.BarCodeAuth).equals("2")) {
			tempCheckDetail = sqlOutStockDataSqlite.getDetailData(checkDetail.strGdBar);
		} else {
			tempCheckDetail = sqlOutStockDataSqlite.GetSpeCheckRecord(strShopID,strCheckID, strPositionID, checkDetail.strGdBar);
		}
		...
		if(!isBtn&&checkDetail.dCheckNum==0){//当为累加显示，修改数量，且输入0时。会出现清空该项
			...
			if (mCheckDetailList.size() > 0) {
				sqlOutStockDataSqlite.SaveCheckDetail(dOriginalNum, checkDetail, 1,flag, mCheckDetailList.get(itemSelected).dNum,isBtn);
			} else {
				sqlOutStockDataSqlite.SaveCheckDetail(dOriginalNum, checkDetail, 1,flag, 0,isBtn);
			}
		}
...
//		}
		...
		if (requestCode == REQUEST_INPUTPSW_CODE) {
			...
			sqlOutStockDataSqlite.InsertCheckDetail(checkDetail, sp.getString(ConfigEntity.UsingBarCodeKey, ConfigEntity.UsingBarCode),sp.getString(ConfigEntity.BarCodeAuthKey,ConfigEntity.BarCodeAuth), tempStatus, sp.getString(ConfigEntity.IsPutInNumKey,ConfigEntity.IsPutInNum), 1);
			refreshListView();
	...
	public void refreshListView() {
		...
		tv_checknum.setText(String.valueOf(sqlOutStockDataSqlite.GetSpecifiedCheckIDSum(strShopID, strCheckID, strPositionID)));
//		tv_qty.setText(String.valueOf(sqlOutStockDataSqlite.GetSpecifiedPositionIdSum(strShopID, strCheckID, strPositionID)));
