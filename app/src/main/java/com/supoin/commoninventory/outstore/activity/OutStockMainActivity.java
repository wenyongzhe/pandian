...
import com.supoin.commoninventory.outstore.adapter.OutStockListAdapter;
...
public class OutStockMainActivity extends Activity implements OnItemClickListener {
	...
	private OutStockListAdapter mStockOutListAdapter;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		CustomTitleBar.getTitleBar(OutStockMainActivity.this, R.string.bar_store_out, false);
		setContentView(R.layout.activity_out_stock_main);
		sqlOutStockDataSqlite = new SQLOutStockDataSqlite(OutStockMainActivity.this,true);
...
//		et_PositionID = (EditText) findViewById(R.id.stockPlace);
		...
		scanOperate.onCreate(OutStockMainActivity.this, R.raw.scanok);
		scanOperate.openScannerPower(is_canScan);
		mMainSummaryInfoList = sqlOutStockDataSqlite.GetSummaryList(strShopID);
		
		//存在货位和编号时，加载到ListView中。否则隐藏ListView，显示empty_list_view
		if (mMainSummaryInfoList != null && mMainSummaryInfoList.size() > 0) {
			mStockOutListAdapter = new OutStockListAdapter(
					OutStockMainActivity.this, mMainSummaryInfoList);
		...
		mListView.setOnItemClickListener(new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> arg0, View arg1,
				...
				mMainSummaryInfoList = sqlOutStockDataSqlite
				...
				strPositionID = mMainSummaryInfoList.get(position)
						.getPositionID();
				Intent intent = new Intent(OutStockMainActivity.this,
			...
			public boolean onEditorAction(TextView v, int actionId,
				...
				if (actionId == EditorInfo.IME_ACTION_DONE
					...
					if (TextUtils.isEmpty(strCheckID)) {
						AlertUtil.showAlert(OutStockMainActivity.this, R.string.dialog_title,
...
//						.toUpperCase();
				strPositionID = "";
				if (TextUtils.isEmpty(strCheckID)) {
					AlertUtil.showAlert(OutStockMainActivity.this, R.string.dialog_title, bianhao+AlertUtil.getString(R.string.cannot_empty),
							R.string.ok, new View.OnClickListener() {
				 ...
				 * 取消判断 货位
				if (TextUtils.isEmpty(strPositionID)) {
					AlertUtil.showAlert(OutStockMainActivity.this,
							R.string.dialog_title, huowei+AlertUtil.getString(R.string.document_empty),
							R.string.ok, new View.OnClickListener() {
								@Override
								public void onClick(View v) {
...
//				strPositionID = et_PositionID.getText().toString().trim()
//						.toUpperCase(); // 库位编号
				final List<MainSummaryInfo> tempSummaryList = sqlOutStockDataSqlite
						.GetSummaryList(strCheckID, strPositionID, strShopID);
				if (tempSummaryList != null && tempSummaryList.size() > 0) {
					//存在该盘点单据
					AlertUtil.showAlert(OutStockMainActivity.this,
							R.string.dialog_title, R.string.document_chuku_exists,
							R.string.ok, new View.OnClickListener() {
								@Override
								public void onClick(View v) {
									AlertUtil.dismissDialog();
									//保存数据到数据库中
									sqlOutStockDataSqlite.SaveCheckMain(strShopID,
									...
									if (mMainSummaryInfoList.size() == 1) {
										mStockOutListAdapter = new OutStockListAdapter(
												OutStockMainActivity.this,
												mMainSummaryInfoList);
										mListView
												.setAdapter(mStockOutListAdapter);
										empty_list_view
												.setVisibility(View.GONE);
										mListView.setVisibility(View.VISIBLE);
									} else {
										mStockOutListAdapter
...
//									finish();
					...
					//保存数据到数据库中
					sqlOutStockDataSqlite.SaveCheckMain(strShopID, strCheckID,
							strPositionID);
					mMainSummaryInfo.setCheckID(strCheckID);
					...
					if (mMainSummaryInfoList.size() == 1) {
						...
						mStockOutListAdapter.setList(mMainSummaryInfoList);
						mListView.setAdapter(mStockOutListAdapter);
					}
