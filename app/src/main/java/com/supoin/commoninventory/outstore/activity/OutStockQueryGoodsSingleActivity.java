...
public class OutStockQueryGoodsSingleActivity extends Activity implements
		OnClickListener {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		CustomTitleBar
				.getTitleBar(OutStockQueryGoodsSingleActivity.this, AlertUtil.getString(R.string.query_outstock), false);
		setContentView(R.layout.activity_out_stock_query_goods_single);
		...
		sqlStockDataSqlite = new SQLOutStockDataSqlite(
				OutStockQueryGoodsSingleActivity.this, true);
		scanOperate = new ScanOperate();
		scanOperate.onCreate(OutStockQueryGoodsSingleActivity.this, R.raw.scanok);
	...
	private void setData() {
		...
		arrDisplayItem = sp.getString(ConfigEntity.DisplayItemsOutKey,
				ConfigEntity.DisplayItemsOut).split(",");
		// 显示选项的导入串
					...
					public boolean onEditorAction(TextView v, int actionId,
						...
						if (actionId == EditorInfo.IME_ACTION_DONE
							...
							if (strBar.trim().length() < 1) {
								AlertUtil.showAlert(
										OutStockQueryGoodsSingleActivity.this, R.string.dialog_title,
	...
	public int ExecuteCutReserveBar(String fullBar, boolean bWithPrompt) {
		...
		if (strLengthCutOrKeepBarCode.equals("1")) // 截取后位数
		{
			if (Integer.parseInt(strLengthCutOrKeepBarCodeNum) >= strBar
					.length()) {
				if (bWithPrompt) {
					AlertUtil.showAlert(OutStockQueryGoodsSingleActivity.this, R.string.warning,
	...
	private void KeyDownOK() {
		...
		if (strBarCodeAuth.equals("1")) {	//条码验证
			...
			if (gdInfo == null) {
				...
				contentList.add("");
				setDisplayContent();
				AlertUtil.showToast("无此条码", OutStockQueryGoodsSingleActivity.this);
				...
				if (qty != 0) {
					...
					contentList.add("");
					setDisplayContent();
					AlertUtil.showToast(AlertUtil.getString(R.string.barCode_not_exist), OutStockQueryGoodsSingleActivity.this);
					tv_qty.setText("0");
	...
	public void onClick(View v) {
		...
		case R.id.btn_ok:
			strBar = et_goodsBar.getText().toString().trim();
			if (strBar.trim().length() < 1) {
				AlertUtil.showAlert(OutStockQueryGoodsSingleActivity.this, R.string.dialog_title,
			...
			KeyDownOK();
			//关闭软键盘
			InputMethodManager inputMethodManager1 = (InputMethodManager) getSystemService(OutStockQueryGoodsSingleActivity.INPUT_METHOD_SERVICE);
		...
		case R.id.btn_cancl:
			...
			//关闭软键盘
			InputMethodManager inputMethodManager = (InputMethodManager) getSystemService(OutStockQueryGoodsSingleActivity.INPUT_METHOD_SERVICE);
